...
# Remove non-adaptive weak TFBSs

In this mode, the program replays mutation and output TRNs after removing non-adaptive weak TFBSs. The program searches for non-adaptive weak TFBSs in four types of TFBSs that can form I1FFLs and NFBLs (see manuscript for details). The program will remove all instances of a type of TFBSs, and if this does not significantly reduce the fitness of TRN, then the type of TFBSs are considered non-adaptive. After removing all four types of non-adaptive weak TFBSs, the program will output a new TRN and the number of motifs. 
To enable the perturbation mode, following these steps:

1. Modify line 25 of netsim.h to 
```c
#define PERTURB 1
```
2. Specify how much reduction in fitness is considered significant by modify line 47 of netsim.h:
```c
#define CUT_OFF_NONADAPTIVE_TFBS -0.01 //default is 1% reduction in fitness 
``` 
...
Because the program needs to measure the fitness of many TRNs, it is recommended to run the program with multiple CPUs (usually take 10 cpus two days). See **readme_output.pdf** in folder **output_sample** for the output files.
...
Before running this mode, users must first run “Remove non-adaptive weak TFBSs” (see above).
To classify mutations, following these steps:
1. Set line 24 and 62 of netsim.h to 1:
```c
#define PHENOTYPE 1
#define CHARACTERIZE_ALL_MUT 1
```
...
# Output expression time course of genes at a given evolutionary step
...
1. Modify line 24 of netsim.h to
```c
#define PHENOTYPE 1
```
and line 54 of netsim.h to
```c
...
# Output expression time course of the effector during the first N evolutionary steps
This mode is similar to the last mode, but it only outputs the expression time course of the effector. 

 . To enable this mode, following these steps:

1. Modify line 24 of netsim.h to
```c
#define PHENOTYPE 1
```
and line 55 of netsim.h to
```c
#define SAMPLE_EFFECTOR_EXPRESSION_LVL 1
```
2. Copy accepted_mutaton_81.txt file (see folder **output_sample**) to result. 
