...
int MAXELEMENTS=100; 
int min_act_to_transcribe[N_THREADS][MAX_MODE];
...
const float MIN_PICDISASSEMBLE = 0.02;
const float MAX_PICDISASSEMBLE = 2.0;
...
const float MIN_mRNA_decay = 4.6e-3;        /* 150 min */
const float MAX_protein_decay = 0.069;  /* Wang, 2002 PNAS 99:5860 */
const float MIN_protein_decay = 4.6e-3; 
...
const float lumped_kon=5.0e-6; 
//                            1.0e-2/6.02e23*1.0e6/3.0e-15;   /* use molecules of a tf to calculate on rate*/ 
                                                            /* kon=1.0e-2 uM^-1*s^-1 */
                                                            /* vol_nuc = 3.0e-15 l^3 */
                                                            /* NA = 6.02e23/1.0e6 convert number of molecules to umol*/                                                            
const float koff=1.0e-2; /*s^-1, this is the initial value of koff. Assuming the average Kd of tf is 10 nM*/
const float MIN_koff= 1.0e-4; /*s^-1, Fordecy 2010 Nat Biotech 28:970 reported Kd of 10 uM. 
                               * Nalefski 2006 Biochem 45:13794 reported 14pM*/
const float MAX_koff= 1.0; /*s^-1*/
const float non_specific_koff=1.0e1; /*s^-1*/; 
...
const float MIN_INTERVAL_TO_UPDATE_PACT=0.5; /* min */
const float MAX_CHANGE_IN_PACT=0.1;
/* end default options */ 

/*Mutations*/
float SUBSTITUTION = 0.33e-9+0.02e-9; /* susbstitution rate per site per cell division. Lynch 2008*/
float INDEL = 0.0;      /* indel rate per site per cell division Lynch 2008 Indels are no longer modeled, but their mutation rates is accounted 
                         * by higher substitution rate */
...
float max_inset=3.0;
float max_delet=3.0;
float sd_mut_effect=1.17; /* 95% of mutations increase or decrease kinetic rates by at most 10 folds*/
float bias_in_mut=0.301; /*mutations to kinetic constants are x*e^N(bias_in_mut,sd_mut_effect). */


/*Cell growth and development*/
//int avg_protein_number = 10000;
float penalty = 2.0e-7; /* reduction in growth rate per protein molecule*/
...
float cost_term=1.0e-3;      /* this determined the cost of translation */                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
float penalty_of_extra_copies=0.0;       /* this is the penalty for having more copies than the MAX_COPIES.
...
float signal_strength=100000.0; /* number of each signal protein */
...
//  float hc, Ltf;
  gpeak = 0.00462*growth_rate_scaling;  /* in min^-1 based on doubling time of 150 min: ln(2)/(150 min)=0.00462 */
//  gpeak_b = 0.005776*growth_rate_scaling;
  Pp_s = 100000.0;
//  Pp_b = 20000;              /* mean gene expression of all proteins is 12064.28 */
float Ltf= 1000;               /* mean gene expression of only TFs is 1418 */
...
//  gmax_b = gpeak+hc*10*Ltf;  
  gmax_a=gpeak+(Pp_s+10.0*Ltf)*0.023*h;
  gmax_b=gpeak+0.023*h*10.0*Ltf;
//   gmax_b=gpeak;
                                ...
                                RngStream RS)
{
    ...
    {  
            ...
            genotype->min_act_to_transc[i]=RngStream_RandInt(RS,1,2);
        #else
            ...
            while (genotype->mRNAdecay[i]<MIN_mRNA_decay || genotype->mRNAdecay[i]>MAX_mRNA_decay)
                genotype->mRNAdecay[i] = exp(0.4909*gasdev(RS)-3.20304);
            /* protein decay */
            genotype->proteindecay[i]=-1.0;
            while (genotype->proteindecay[i]<MIN_protein_decay || genotype->proteindecay[i]>MAX_protein_decay) 
            {
            ...
            while (genotype->translation[i]<MIN_TRANSLATION || genotype->translation[i]>MAX_TRANSLATION)
                genotype->translation[i] = exp(0.7406*gasdev(RS)+4.56);
        #else        
            genotype->koff[i]=koff;        
            genotype->mRNAdecay[i]=0.069; //half life is about 10min
 ...
 *
 */
void set_binding_sites(Genotype *genotype,int *N_BS_per_gene, int (*tf_id_per_site)[5], int (*hindrance_per_site)[5], float (*koff_per_site)[5])
{
    int i,j;
    ...
    for(i=2;i<genotype->ngenes;i++)
    {
        ...
        for(j=0;j<N_BS_per_gene[i];j++)
        {
            ...
            genotype->all_binding_sites[i][j].Koff=koff_per_site[i][j];
...
void calc_all_binding_sites_copy(Genotype *genotype, int gene_id)
{
    ...
    int act_BS[MAXELEMENTS][2],rep_BS[MAXELEMENTS][2],N_act_BS,N_rep_BS;
//    int either_BS[MAXELEMENTS][2],N_either_BS;
    int N_hindered_BS=0;
    ...
    {  
        ...
        {    
            ...
            {  
                ...
                genotype->all_binding_sites[gene_id][N_binding_sites].Koff = genotype->koff[k]*pow(non_specific_koff/genotype->koff[k],(float)(TF_ELEMENT_LEN-match)/(TF_ELEMENT_LEN-NMIN+1));                
...
//                genotype->all_binding_sites[gene_id][N_binding_sites].compressed = 0;
            ...
            else /*find BS on the non-template strand*/
            {
                ...
                if (match_rc >= NMIN)
                {
                    ...
                    /************************************************************************************************************/
                    genotype->all_binding_sites[gene_id][N_binding_sites].tf_id = k;
                    genotype->all_binding_sites[gene_id][N_binding_sites].Koff = genotype->koff[k]*pow(non_specific_koff/genotype->koff[k],(float)(TF_ELEMENT_LEN-match_rc)/(TF_ELEMENT_LEN-NMIN+1));                   
                    genotype->all_binding_sites[gene_id][N_binding_sites].BS_pos = i;
                    genotype->all_binding_sites[gene_id][N_binding_sites].mis_match = TF_ELEMENT_LEN-match;
//                    genotype->all_binding_sites[gene_id][N_binding_sites].compressed = 0;
    ...
    /* calc max_un_hindered_sites */
    /* first make lists of two types of BS*/    
    N_act_BS=1;
    N_rep_BS=1;
//    N_either_BS=1;
    for(i=0;i<N_binding_sites;i++)
...
//    for(i=1;i<N_either_BS;i++)
//    {
//        j=i-1;
//        while(j!=0 && genotype->all_binding_sites[gene_id][either_BS[i][0]].BS_pos-genotype->all_binding_sites[gene_id][either_BS[j][0]].BS_pos<TF_ELEMENT_LEN+2*HIND_LENGTH)j--;
//        either_BS[i][1]=either_BS[j][1]+1;
//    }
    genotype->max_unhindered_sites[gene_id][1]=act_BS[N_act_BS-1][1];
    genotype->max_unhindered_sites[gene_id][2]=rep_BS[N_rep_BS-1][1];
//    genotype->max_unhindered_sites[gene_id][0]=either_BS[N_either_BS-1][1];
}
...
float calc_TF_dist_from_all_BS( AllTFBindingSites *BS_info,
                                ...
                                int thread_id)
{
    int approximation_x=max_unhindered_sites[1]+1;
    int approximation_y=max_unhindered_sites[2]+1;
    double ratio_matrices[max_N_hindered_BS+1][approximation_y][approximation_x];   
    double transition_matrix[approximation_y][approximation_x];
    double sum,prob_act_over_rep=0.0;    
    double product_of_freq;    
    ...
    for(i=0;i<max_N_hindered_BS+1;i++)
    {
        for(j=0;j<approximation_y;j++) 
        {
            for(k=0;k<approximation_x;k++)
                ratio_matrices[i][j][k]=0.0;
        }
    }
    for(j=0;j<approximation_y;j++)
            ...
            case 1: // a BS of activators              
                if(m-BS_info[m].N_hindered!=0)
                {
                  ...
                  // the 10th record is at 9. Note in gcc mod does not follow the conventional mathematical definition 
                    ...
                    for(i=0;i<approximation_y;i++)
                    {
                        transition_matrix[i][0]=0.0; 
                        for(j=1;j<approximation_x;j++)
//                        for(j=1;j<((approximation_x<max_unhindered_sites[0]+1-i)?approximation_x:max_unhindered_sites[0]+1-i);j++)
                ...
                else
                {
                    for(i=0;i<approximation_y;i++)
                    {
                        for(j=0;j<approximation_x;j++)
//                        for(j=0;j<((approximation_x<max_unhindered_sites[0]+1-i)?approximation_x:max_unhindered_sites[0]+1-i);j++)
                            transition_matrix[i][j]=0.0;
                    }
                ...
                for(i=0;i<approximation_y;i++)
                {
                    for(j=0;j<approximation_x;j++) 
//                    for(j=0;j<((approximation_x<max_unhindered_sites[0]+1-i)?approximation_x:max_unhindered_sites[0]+1-i);j++)
                        ratio_matrices[pos_next_record][i][j]=BS_info[m].Koff*ratio_matrices[pos_of_last_record][i][j]+product_of_freq*transition_matrix[i][j];
                }
                break;
...
//        for(j=0;j<((approximation_x<max_unhindered_sites[0]+1-i)?approximation_x:max_unhindered_sites[0]+1-i);j++)        
    ...
    for(i=0;i<approximation_y;i++)
    {    
        j=(min_act_to_transcr>min_act_to_transcribe[thread_id][i])?min_act_to_transcr:min_act_to_transcribe[thread_id][i];
        for(;j<approximation_x;j++)
//        for(;j<((approximation_x<max_unhindered_sites[0]+1-i)?approximation_x:max_unhindered_sites[0]+1-i);j++)        
            prob_act_over_rep+=ratio_matrices[pos_next_record][i][j];        
    } 
                        ...
                        RngStream RS)
{
    ...
    if(duration_of_burn_in_growth_rate!=0.0)
        ...
        add_fixed_event(-1,(float)TIME_INFINITY,&(state->burn_in_growth_rate),&(state->burn_in_growth_rate_last));
...
void set_env(CellState *state, char env, float duration_env1, float duration_env2)
{
    ...
    if(env=='A')
        state->protein_number[N_SIGNAL_TF-1]=signal_strength;
    else            
        state->protein_number[N_SIGNAL_TF-1]=basal_signal_strength;
    #if N_SIGNAL_TF==2
...
void calc_all_rates(Genotype *genotype,
                    ...
                    GillespieRates *rates, 
                    float t,
                    int UPDATE_WHAT,int thread_id)
{
    int i,cluster_id,too_much_error;
    float salphc;
//    float protein_decay;
    int concurrent;
    float t_to_update_Pact,interval_to_update_Pact;
...
//            protein_decay = genotype->proteindecay[i] >= protein_aging ? genotype->proteindecay[i] : protein_aging;            
            salphc = state->mRNA_cyto_num[i] * genotype->translation[i]/(genotype->proteindecay[i]);
...
//        rates->transport_rate[i] = KRNA * state->mRNA_nuclear_num[i];
//        rates->transport += rates->transport_rate[i];
        rates->mRNAdecay_rate[i] = genotype->mRNAdecay[i] * (state->mRNA_cyto_num[i] + state->mRNA_transl_cyto_num[i]);
        rates->mRNAdecay += rates->mRNAdecay_rate[i];        
    }
    
    for (i=N_SIGNAL_TF; i < genotype->ngenes; i++) 
    {    
        #if NO_REGULATION // no expression of the regular TFs
...
//    rates->subtotal+=rates->transport;
    ...
    if(UPDATE_WHAT==UPDATE_PACT || UPDATE_WHAT==INITIALIZATION)
        interval_to_update_Pact=MIN_INTERVAL_TO_UPDATE_PACT;
    else
    {
        too_much_error=0;        
        for(i=N_SIGNAL_TF;i<genotype->ngenes;i++)
...
//            }
        ...
        if(!too_much_error)
            interval_to_update_Pact=(t-state->last_event_t)*2.0;        
        else        
            interval_to_update_Pact=(t-state->last_event_t)*0.67;  
        
        if(interval_to_update_Pact<MIN_INTERVAL_TO_UPDATE_PACT)
            interval_to_update_Pact=MIN_INTERVAL_TO_UPDATE_PACT;
    }    
    
    t_to_update_Pact=t+interval_to_update_Pact;
...
int does_fixed_event_end(CellState *state, float t) 
{
    ...
    if( state->mRNA_transl_init_time_end == NULL && 
        state->mRNA_transcr_time_end==NULL && 
        state->signalB_starts_end == NULL && 
        state->signalA_starts_end == NULL && 
        state->burn_in_growth_rate == NULL)
    ...
    {
        t1 = state->mRNA_transcr_time_end ? state->mRNA_transcr_time_end->time : TIME_INFINITY;
        t2 = state->mRNA_transl_init_time_end ? state->mRNA_transl_init_time_end->time : TIME_INFINITY;
        t3 = state->signalB_starts_end? state->signalB_starts_end->time : TIME_INFINITY;
        t4 = state->signalA_starts_end ? state->signalA_starts_end->time : TIME_INFINITY;
	t5 = state->burn_in_growth_rate ? state->burn_in_growth_rate->time : TIME_INFINITY;
        t6 = state->t_to_update_Pact;
                            ...
                            char *env)
{
    ...
    *dt = state->mRNA_transl_init_time_end->time - t;         /* make dt window smaller */
    ...
    state->konvalues[gene_id][KON_SALPHAC_INDEX] = state->mRNA_cyto_num[gene_id] * genotype->translation[gene_id] / state->konvalues[gene_id][KON_PROTEIN_DECAY_INDEX];
                        ...
                        int clone_type)
{
    ...
    if(clone_type!=MUT_KCONST) /* not a mutation in kinetic constants*/
    {
        ...
        i=0;
        while(genotype_clone->cisreg_cluster[i][0]!=-1 && i<NGENES)
        {
            j=0;
            while(genotype_clone->cisreg_cluster[i][j]!=-1 && j<NGENES)
            {
        ...
        /*then copy from orig*/
        i=0;
        while(genotype_orig->cisreg_cluster[i][0]!=-1 && i<NGENES)
        {
            j=0;
            while(genotype_orig->cisreg_cluster[i][j]!=-1 && j<NGENES)
            {
...
//            state->t_to_update_Pact=*t+state->interval_to_update_Pact;
//            calc_all_rates(genotype, state, rates, *t, INITIALIZATION);
...
{     
	...
	case 5: /* finishing burn-in growth rate*/
            *dt=duration_of_burn_in_growth_rate-t;     
            ...
            break;
        case 6: /* update force to update Pact*/
            *dt=state->t_to_update_Pact-t;
            update_protein_number_cell_size(genotype, state, rates, *dt, t, *env, end_state, error, mut_step, mut_record);          
            break;
#if CAUTIOUS
        FILE *fp;
...
//        int ID=0;
        ...
        {	 
            ...
            initialize_cell(&state_offspring, genotype_offspring.ngenes, genotype_offspring.nproteins,
                            genotype_offspring.protein_pool,genotype_offspring.mRNAdecay, mRNA, protein, RS_parallel[ID]);
            set_env(&state_offspring,env,env1_t_signalA,env1_t_signalB);         
            t = 0.0;
            calc_all_rates(&genotype_offspring, &state_offspring, &rate_offspring, t, INITIALIZATION,ID); 
                                    ...
                                    float t) 
{
        ...
        if((t1 <= t2) && (t1 <= t) && (t1 <= t3) && (t1 <= t4) && (t1<=t5) && (t1<=t6) && (t1<=t7))
	{
        ...
        {
            if ((t2 <= t1) && (t2 <= t) && (t2 <= t3) && (t2 <= t4) && (t2<=t5) && (t2<=t6) && (t2<=t7)) 
            {
                retval = 2;
            }
            else
            {
                if ((t3 <= t1) && (t3 <= t) && (t3 <= t2) && (t3 <= t4) && (t3<=t5) && (t3<=t6) && (t3<=t7)) 
                {
                    retval = 3;
                }
                else
                {
                    if ((t4 <= t1) && (t4 <= t) && (t4 <= t2) && (t4 <= t3) && (t4<=t5) && (t4<=t6) && (t4<=t7)) 
                    {
                        retval = 4;
                    }
                    else
                    {
			if((t5 <= t1) && (t5 <= t) && (t5 <= t2) && (t5 <= t3) && (t5<=t4) && (t5<=t6) && (t5<=t7))
			{
                            retval = 5;
			}
			else
                        {
                            if((t6 <= t1) && (t6 <= t) && (t6 <= t2) && (t6 <= t3) && (t6<=t4) && (t6<=t5) && (t6<=t7))
                            {
                                retval=6;
                            }
                            else
                            {
                                if((t7 <= t1) && (t7 <= t) && (t7 <= t2) && (t7 <= t3) && (t7<=t4) && (t7<=t5) && (t7<=t6))
                                    retval=7;
                                else
                                    retval = 0;
                            }
	...
	case 5: /* finishing burn-in growth rate*/
        ...
        case 6:
            *dt=state->sampling_point_end->time-t;
            ...
            break;
        case 7: /* update force to update Pact*/
            *dt=state->t_to_update_Pact-t;
            update_protein_number_cell_size(genotype, state, rates, *dt, t, *env, end_state, NULL, 0, NULL);
...
{     
    float phenotypeA[N_REPLICATES][genotype->ngenes][159];
    float phenotypeB[N_REPLICATES][genotype->ngenes][159];
    float fitnessA[N_REPLICATES][159];
    float fitnessB[N_REPLICATES][159];
    int l,m,n;
...
//        #pragma omp critical
        ...
        {             
            ...
            initialize_cell(&state_offspring, genotype_offspring.ngenes, genotype_offspring.nproteins,
                            genotype_offspring.protein_pool,genotype_offspring.mRNAdecay, mRNA, protein, RS[ID]);
            set_env(&state_offspring,env,env1_t_signalA,env1_t_signalB);
//            state_offspring.interval_to_update_Pact=MIN_INTERVAL_TO_UPDATE_PACT;
            t = 0.0;
            calc_all_rates(&genotype_offspring, &state_offspring, &rate_offspring, t, INITIALIZATION,ID);           
...
{
    int pos_n, pos_g,i,N_BS_bf_mutation,flag_difference;
    ...
    Genome= &genotype->cisreg_seq[0][0];
        ...
        /*calc bs before substitution*/
        calc_all_binding_sites_copy(genotype,pos_g);
        N_BS_bf_mutation=genotype->binding_sites_num[pos_g];
        for(i=0;i<N_BS_bf_mutation;i++)
        {
            container[i].BS_pos=genotype->all_binding_sites[pos_g][i].BS_pos;
            container[i].tf_id=genotype->all_binding_sites[pos_g][i].tf_id;
            container[i].mis_match=genotype->all_binding_sites[pos_g][i].mis_match;
        }
    ...
    calc_all_binding_sites_copy(genotype,pos_g);    
    if(N_BS_bf_mutation!=genotype->binding_sites_num[pos_g])
    {
        update_cisreg_cluster(genotype,pos_g,'s',NULL,-1,-1);  
    }
    else
    {
        flag_difference=0;
        for(i=0;i<N_BS_bf_mutation;i++)
        {
            if(container[i].BS_pos!=genotype->all_binding_sites[pos_g][i].BS_pos ||
                container[i].tf_id!=genotype->all_binding_sites[pos_g][i].tf_id ||
                container[i].mis_match!=genotype->all_binding_sites[pos_g][i].mis_match)
            {    
                flag_difference=1;
...
{    
    char *Genome;
    int i, N_BS_bf_mutation,pos_g,flag_difference;
    ...
    Genome= &genotype->cisreg_seq[0][0];
    Genome[mut_record->pos_n]=mut_record->nuc_diff[1]; 
    calc_all_binding_sites_copy(genotype,pos_g);    
    if(N_BS_bf_mutation!=genotype->binding_sites_num[pos_g])
    {
        update_cisreg_cluster(genotype,pos_g,'s',NULL,-1,-1);  
...
//    genotype->recalc_TFBS[pos_g]=1;  /*recalc TFBS*/
//    genotype->clone_info[pos_g]=1;   /*restore info in clone_cell*/
//    update_cisreg_cluster(genotype,pos_g,'i',NULL,-1,-1);    
//    
//    /*record mutation info*/
//    mut_record->pos_n=pos_n;
//    mut_record->pos_g=pos_g;    
...
//    genotype->recalc_TFBS[pos_g]=1;  /*recalc TFBS*/
//    genotype->clone_info[pos_g]=1;   /*restore info in clone_cell*/
//    update_cisreg_cluster(genotype,pos_g,'p',NULL,-1,-1);  
}
...
int mut_whole_gene_deletion(Genotype *genotype, Mutation *mut_record, RngStream RS) // any gene can be deleted
{
    ...
    if(genotype->ntfgenes-N_SIGNAL_TF>1) 
    {
    ...
    else
    {
        while(1)
        {
            ...
            if(genotype->which_protein[pos_g]!=genotype->nproteins-2) // the only regular tf must be nproteins-2
                break;
        }
    }
    ...
    update_cisreg_cluster(genotype,pos_g,'w',NULL,-1,-1);  
    genotype->ngenes--;  
    return 0;
}
...
{    
    ...
    /*if the original selection genes are to be deleted, we find one of their duplicated genes and copy it to 
     the default selection genes' position. Then we deleted the duplicated gene. For example, assuming ngenes=10,
     gene 9 is selection gene A and is marked for deletion. We know gene 5 was duplicated from gene 8, so we 
     overwrite gene 9 with all the info of gene 5, then we delete gene 5 (the puppet). We go through these troubles to keep 
     the structure of genotype information.*/
    if(pos_g==genotype->ngenes-1)
    {
        cluster_id=genotype->which_cluster[pos_g];
...
int mut_duplication(Genotype *genotype, Mutation *mut_record, RngStream RS) //any gene can be duplicated
{
    int pos_g, pos_g_copy, i, protein_id;
    char *temp1, *temp2; 
    
    if(genotype->protein_pool[genotype->nproteins-1][0][0]>=EFFECTOR_GENES) // too many effector gene copies
    {     
        ...
        pos_g=genotype->ngenes-1;
        while(genotype->which_protein[pos_g]==genotype->nproteins-1)
            pos_g=RngStream_RandInt(RS,N_SIGNAL_TF,genotype->ngenes-1);
    }
    else
    {
        if(genotype->ntfgenes>=TFGENES) // too many tf gene copies
        {
            while(1)
            {
                pos_g=RngStream_RandInt(RS,N_SIGNAL_TF,genotype->ngenes-1);
                if(genotype->which_protein[pos_g]==genotype->nproteins-1)
                    break;
            }    
        }            
        else
    ...
    update_cisreg_cluster(genotype,pos_g,'d',NULL,-1,-1);
    /* update gene numbers*/  
...
void mut_binding_sequence(Genotype *genotype, Mutation *mut_record, RngStream RS)
{
    ...
    int group[NGENES][NGENES],genes_in_cluster[NGENES];
    int len,no_difference,gene_head,gene_to_be_sorted;
    int N_groups,counter,pos,j,k;   
    ...
    {        
        ...
        while(genotype->cisreg_cluster[i][len]!=-1)
        {
            genes_in_cluster[len]=genotype->cisreg_cluster[i][len];
            len++;
        }
        /*the while loop below sort genes in cluster into groups based on whether they have the same BS distributions*/
        while(len>0)
        {
            group[N_groups][0]=genes_in_cluster[0];
            counter=1;
            pos=0;
            while(pos<len)
            {
                no_difference=1;
                gene_to_be_sorted=genes_in_cluster[pos];
                gene_head=genes_in_cluster[0];
                if(genotype->binding_sites_num[gene_to_be_sorted]==genotype->binding_sites_num[gene_head])
                {
                    for(j=0;j<genotype->binding_sites_num[gene_head];j++)
                    {
                        if(genotype->all_binding_sites[gene_head][j].BS_pos!=genotype->all_binding_sites[gene_to_be_sorted][j].BS_pos ||
                            genotype->all_binding_sites[gene_head][j].tf_id!=genotype->all_binding_sites[gene_to_be_sorted][j].tf_id ||
                            genotype->all_binding_sites[gene_head][j].mis_match!=genotype->all_binding_sites[gene_to_be_sorted][j].mis_match)
                        {
                            no_difference=0;
                            break;
                        }
                ...
                if(no_difference)
                {
                    group[N_groups][counter]=genes_in_cluster[pos];
                    counter++;
                    for(j=pos;j<len-1;j++)
        ...
        if(N_groups!=1)
            update_cisreg_cluster(genotype,-1,'c',group,N_groups,i);
        i++;
    }
...
{    
    ...
    tf_seq[pos_n]=mut_record->nuc_diff[1];
...
void mut_kinetic_constant(Genotype *genotype, Mutation *mut_record, float kdis[NUM_K_DISASSEMBLY],RngStream RS)
{
    ...
    #else
        temp=genotype->pic_disassembly[pos_g]*exp(sd_mut_effect*gasdev(RS)+bias_in_mut);  
        while(temp<MIN_PICDISASSEMBLE || temp>MAX_PICDISASSEMBLE)
            temp=genotype->pic_disassembly[pos_g]*exp(sd_mut_effect*gasdev(RS)+bias_in_mut);
        genotype->pic_disassembly[pos_g]=temp;
    #endif        
        /*record mutation info*/
    ...
    else 
    {
        ...
        #else            
            temp=genotype->mRNAdecay[pos_g]*exp(sd_mut_effect*gasdev(RS)+bias_in_mut); 
            while(temp>MAX_mRNA_decay || temp<MIN_mRNA_decay)
                temp=genotype->mRNAdecay[pos_g]*exp(sd_mut_effect*gasdev(RS)+bias_in_mut);
            genotype->mRNAdecay[pos_g]=temp;
        #endif
            /*record mutation info*/
        ...
        else 
        {
        ...
        #else
                temp=genotype->translation[pos_g]*exp(sd_mut_effect*gasdev(RS)-bias_in_mut);
                while(temp>MAX_TRANSLATION || temp< MIN_TRANSLATION)
                    temp=genotype->translation[pos_g]*exp(sd_mut_effect*gasdev(RS)-bias_in_mut);
                genotype->translation[pos_g]=temp;
        #endif
            ...
            {  
                ...
                if(random1<=proportion_mut_protein_decay/total_mut_flux)
                {
                    ...
                    #else
                        temp=genotype->proteindecay[pos_g]*exp(sd_mut_effect*gasdev(RS)+bias_in_mut);  
                        while(temp>MAX_protein_decay || temp<MIN_protein_decay)
                            temp=genotype->proteindecay[pos_g]*exp(sd_mut_effect*gasdev(RS)+bias_in_mut); 
                        genotype->proteindecay[pos_g]=temp;
                    #endif
                ...
                {
                    if(genotype->min_act_to_transc[pos_g]==1)
                        genotype->min_act_to_transc[pos_g]=2;
                    else
                        genotype->min_act_to_transc[pos_g]=1;
                    mut_record->kinetic_type=4;
                    mut_record->kinetic_diff=(float)genotype->min_act_to_transc[pos_g];
                }
            } 
        }
    }
...
{    
    ...
    switch (mut_record->kinetic_type)
    {
        ...
        case 3: /* mut protein decay */
            ...
            break; 
        case 4:
            genotype->min_act_to_transc[pos_g]=(int)mut_record->kinetic_diff;
    }
}
...
void mut_identity(Genotype *genotype, Mutation *mut_record, RngStream RS)
{
    ...
    tf_id = RngStream_RandInt(RS,N_SIGNAL_TF,genotype->ngenes-2); // the first signal tf must be an activator, therefore is not subject to mutation

    protein_id=genotype->which_protein[tf_id];    
    while(protein_id==genotype->nproteins-1)
    {
        tf_id = RngStream_RandInt(RS,N_SIGNAL_TF,genotype->ngenes-2); // the signal tf must be an activator, therefore is not subject to mutation
        protein_id=genotype->which_protein[tf_id];
    }    
...
void mut_koff(Genotype *genotype, Mutation *mut_record, RngStream RS)
{
    ...
    unsigned long seeds1[6];
    FILE *fp;
    RngStream_GetState(RS,seeds1);
    fp=fopen("RngSeeds1.txt","a+");
    fprintf(fp,"%lu %lu %lu %lu %lu %lu ",seeds1[0],seeds1[1],seeds1[2],seeds1[3],seeds1[4],seeds1[5]);
    fprintf(fp,"\n");
    fclose(fp);
        ...
        for(j=0;j<genotype->binding_sites_num[i];j++)
        {
            if(genotype->all_binding_sites[i][j].tf_id==tf_id)
                genotype->all_binding_sites[i][j].Koff=genotype->koff[protein_id]*
                        pow(non_specific_koff/genotype->koff[protein_id],(float)genotype->all_binding_sites[i][j].mis_match/(TF_ELEMENT_LEN-NMIN+1));
    ...
    mut_record->pos_g=tf_id;
    mut_record->kinetic_diff=new_koff; 
    ...
    {    
        ...
        switch (mut_record->mut_type)
        {
            ...
            case 'w': // whole gene deletion.          
                redraw_mutation=mut_whole_gene_deletion(genotype,mut_record,RS);
                return_val=MUT_N_GENE;
                break;

            case 'd': // Whole gene duplication also has two versions                  
                redraw_mutation=mut_duplication(genotype,mut_record,RS);
                return_val=MUT_N_GENE;
                break;
...
int reproduce_mutate(Genotype *genotype, Mutation *mut_record,RngStream RS)
{    
    int return_val=0;
//    char temp=mut_record->mut_type;
//    draw_mutation(genotype, &(mut_record->mut_type),RS);
//    mut_record->mut_type=temp;
    switch (mut_record->mut_type)
    {
        case 's': //substitution        		
...
void draw_mutation(Genotype *genotype, char *mut_type, RngStream RS)
{
    ...
    /* duplication rate*/    
    tot_dup_rate=0.0;
    N_genes_to_be_duplicated=genotype->ngenes-N_SIGNAL_TF;
    if(genotype->ntfgenes>=TFGENES)
        N_genes_to_be_duplicated-=genotype->ntfgenes-N_SIGNAL_TF;
    if(genotype->protein_pool[genotype->nproteins-1][0][0]>=EFFECTOR_GENES)
        N_genes_to_be_duplicated-=genotype->protein_pool[genotype->nproteins-1][0][0];
    tot_dup_rate=N_genes_to_be_duplicated*DUPLICATION;
    tot_mut_rate+=tot_dup_rate;     
    
    /* silencing rate*/ 
    if(genotype->ntfgenes-N_SIGNAL_TF>1)
        tot_sil_rate=(genotype->ngenes-1-N_SIGNAL_TF)*SILENCING; /* the signaling gene and one copy of the selection gene cannot be deleted */    
    else
        tot_sil_rate=(genotype->ngenes-2-N_SIGNAL_TF)*SILENCING; /* the signaling gene, one copy of the selection gene, and the gene of the last non-signal tf cannot be deleted */
    tot_mut_rate+=tot_sil_rate;
    
    /* calc total susbtitution rate*/
    tot_subs_rate=(genotype->ngenes-N_SIGNAL_TF)*CISREG_LEN*SUBSTITUTION; /* NA to the signal gene*/
    tot_mut_rate+=tot_subs_rate;
    
    /* indel rate*/
    tot_indel_rate=(genotype->ngenes-N_SIGNAL_TF)*CISREG_LEN*INDEL;    /* NA to the signal gene*/
    tot_mut_rate+=tot_indel_rate;
    
    /* mut in kinetic constants */
    tot_mut_kin_rate=(genotype->ngenes-N_SIGNAL_TF)*MUTKINETIC*(proportion_mut_kdis+proportion_mut_mRNA_decay+proportion_mut_protein_decay+proportion_mut_translation_rate+proportion_mut_cooperation); /* NA to the signal gene*/
    tot_mut_rate+=tot_mut_kin_rate;
    
    /* mut in binding seq*/
    tot_mut_binding_seq_rate=genotype->ntfgenes*MUTKINETIC*proportion_mut_binding_seq; /* NA to the selection genes*/
    tot_mut_rate+=tot_mut_binding_seq_rate;
    
    /* mut in identity*/
    tot_mut_identity_rate=(genotype->ntfgenes-N_SIGNAL_TF)*MUTKINETIC*proportion_mut_identity; /* if there's only one signal tf, then this tf must be an activator*/
...
void update_cisreg_cluster(Genotype *genotype, int gene_id, char mut_type, int new_clusters[NGENES][NGENES], int N_new_clusters, int original_cluster_id)
{
    int cluster_id, cluster_id_copy, i, j, last_cluster;   
    ...
    {       
        ...
        while(genotype->cisreg_cluster[original_cluster_id][i]!=-1)
            genotype->cisreg_cluster[original_cluster_id][i]=genotype->cisreg_cluster[original_cluster_id][i+1];
        i=0;
        while(new_clusters[0][i+1]!=-1)
        {
            genotype->cisreg_cluster[original_cluster_id][i]=new_clusters[0][i+1];
            genotype->which_cluster[new_clusters[0][i+1]]=original_cluster_id;
        }
        ...
        for(i=1;i<N_new_clusters;i++)
        {
            ...
            j=0;
            while(new_clusters[i][j+1]!=-1)
            {
                genotype->cisreg_cluster[last_cluster][j]=new_clusters[i][j+1];
                genotype->which_cluster[new_clusters[i][j+1]]=last_cluster;
            }
            last_cluster++;
        }        
        return;
                        ...
                        RngStream RS)
{
    ...
    for(i=0;i<replay_N_steps;i++)
    {
        ...
        fp=fopen("proportion_c1ffl","a+");        
        fprintf(fp,"%f %f %f %f %f %f %f %f %d %d %d %d %d %d %d %d %d %d %d %d\n",genotype_ori->proportion_motifs[0],
                                                                                    genotype_ori->proportion_motifs[1],
                                                                                    genotype_ori->proportion_motifs[2],
                                                                                    genotype_ori->proportion_motifs[3], 
                                                                                    genotype_ori->proportion_motifs[4],
                                                                                    genotype_ori->proportion_motifs[5], 
                                                                                    genotype_ori->proportion_motifs[6],
                                                                                    genotype_ori->proportion_motifs[7],
                                                                                    genotype_ori->N_motifs[0],
                                                                                    genotype_ori->N_motifs[1],
                                                                                    genotype_ori->N_motifs[2],
                                                                                    genotype_ori->N_motifs[3],
                                                                                    genotype_ori->N_motifs[4],
                                                                                    genotype_ori->N_motifs[5],
                                                                                    genotype_ori->N_motifs[6],
                                                                                    genotype_ori->N_motifs[7],
                                                                                    genotype_ori->N_act_genes,
                                                                                    genotype_ori->N_act_genes_reg_by_env,
                                                                                    genotype_ori->N_act_genes_not_reg_by_env,
                                                                                    genotype_ori->protein_pool[genotype_ori->nproteins-1][0][0]);
        fclose(fp);
                        ...
                        RngStream RS_main)
{
    int i=0;    
    FILE *fp;
    while(genotype_ori->ntfgenes>2 && i<max_mut_steps)
    {
        while(1)
        {
            clone_cell_forward(genotype_ori,genotype_ori_copy,COPY_ALL);
...
{
    FILE *fp;
    int max_mut_steps,run_burn_in,N_tot_trials,first_step;
    ...
    {
        run_burn_in=1;      
        max_mut_steps=BURN_IN;    
        tdevelopment=159.9;                 // global variable
        duration_of_burn_in_growth_rate=0.0;// global variable
        init_env1='A';                      // global variable
        init_env2='A';                      // global variable
        env1_t_signalA=180.0;               // global variable 
        env1_t_signalB=0.0;                 // global variable
        env2_t_signalA=20.0;               // global variable
        env2_t_signalB=60.0;                 // global variable 
        env1_signalA_as_noise=0;            // global variable
        env2_signalA_as_noise=1;            // global variable
    ...
    env1_occurence=0.5;                     // global variable
    env2_occurence=0.5;                     // global variable
    DUPLICATION=1.5e-7;                 /* per gene per cell division (using 120min), excluding chromosome duplication. Lynch 2008*/
    SILENCING = 1.3e-7+0.2e-7;          /* per gene per cell division (120min), excluding chromosome deletion,Lynch 2008. 
                                        * Also account mutations that destroy the function of a gene (1.5 kb): 
                                        *  frameshifting (2/3 of indels), pre_mature stop codon (23/61 codons are 1 subs from stop codon), 
                                        * critical non-synonymous substitituions (50% subs (this number is actually 76%) are non-synonymous and 80% non-synonymous
                                        * are critical) */
                                        /* The two rates were reduced 10 folds (11/26)*/
    if(first_step<BURN_IN)
    {
        fp=fopen(RuntimeSumm,"a");
        ...
        fprintf(fp,"Basal environmental signal strength=%f, environmental signal strength=%f\n",basal_signal_strength,signal_strength);
        fclose(fp);
    }
    evolve_N_steps( genotype_ori, 
                    genotype_ori_copy,
...
//    exit(0);
    ...
    fp=fopen(file_genotype_summary,"r");
    if(fp!=NULL)
    {
        for(i=0;i<replay_N_steps+1;i++)
            fgets(buff_char,200,fp);
       
        fscanf(fp,"%d %d %d %c %f %f %f %f %f %f %f %d %d %d %d\n",
                &buff_int,
                &N_tot_mutations,
                &buff_int,
                &(buff_char[0]),
...
void evolve_N_steps(Genotype *genotype_ori, 
                    Genotype *genotype_ori_copy,
                    int *init_step, 
                    int max_steps, 
                    int *N_tot_trials, 
...
//                                recalc_new_fitness);
        ...
        {	
            ...
            z_score=try_fixation(genotype_ori, genotype_ori_copy, N_REPLICATES*recalc_new_fitness, N_REPLICATES, &fixation, RS_main); 
            if(fixation==1)
            {                    
                fp=fopen(file_genotype_summary,"a+");
                fprintf(fp,"%d %d %d %c %.10f ",i, *N_tot_trials, N_trials,mut_record->mut_type,z_score);
                fclose(fp);
                fp=fopen(file_mutations,"a+");
                fprintf(fp,"%c %d %d '%s' %d %f\n",
                        mut_record->mut_type,    
        ...
        /* marks the last step at which all state of the program is output*/
        fp=fopen("saving_point","w");
        fprintf(fp,"%d %d\n",i,*N_tot_trials);
        fclose(fp);
        
        /* determine if QUICK_BURN_IN is enabled*/
        #if QUICK_BURN_IN
            if(run_burn_in)
            {            
...
//    float koff_per_site[5][5]={{koff,koff,koff,koff,koff},{koff,koff,koff,koff,koff},{koff,koff,koff,koff,koff},{koff,koff,koff,koff,koff},{koff,koff,koff,koff,koff}};
    ...
    float koff_per_site[6][5]={{koff,koff,koff,koff,koff},{koff,koff,koff,koff,koff},{koff,koff,koff,koff,koff},{koff,koff,koff,koff,koff},{koff,koff,koff,koff,koff},{koff,koff,koff,koff,koff}};    
    genotype->ngenes++; 
    ...
    set_binding_sites(genotype,N_BS_per_gene,tf_id_per_site,hindrance_per_site,koff_per_site);
}
#endif
...
{  
    ...
    for(i=0;i<N_THREADS;i++)
    {
        for(j=0;j<MAX_MODE;j++)
        {
            min_act_to_transcribe[i][j]=round(fabs((j-0.31)/0.33)); // determine the number of activators needs for transcription 
    ...
    {   
        ...
        {                     
            ...
            env1_signalA_as_noise=0;    
            env2_signalA_as_noise=1; 
            recalc_new_fitness=10; // make sure its value is smaller than MAX_RECALC_FITNESS
            env1_occurence=0.5;
            env2_occurence=0.5;    
            float GR1[recalc_new_fitness][N_REPLICATES],GR2[recalc_new_fitness][N_REPLICATES];
            ...
            fp=fopen(file_genotype_summary,"a+");
            fprintf(fp,"step N_tot_mut_tried N_mut_tried_this_step fixed_mutation z_score fitness se_fitness avg_GR1 avg_GR2 std_GR1 std_GR2 N_genes N_proteins N_activator N_repressor\n");
...
void find_ffl(Genotype *genotype)
{
    ...
    int found_bs;
    int gene_id,site_id,protein_id,N_copies,N_activators;
//    int repressors[NPROTEINS];
    ...
    int pos_binding_sites_of_j[20],pos_binding_sites_of_k[20],N_binding_sites_of_j,N_binding_sites_of_k;
    int N_all_motifs;
...
//                } 
                ...
                for(j=0;j<N_activators;j++)
                {
                    ...
                    if(pos_binding_sites_of_j[N_binding_sites_of_j-1]-pos_binding_sites_of_j[l]>=TF_ELEMENT_LEN+2*HIND_LENGTH) //check if the first bs hinders the last bs                    
                        hindrance[activators[j]][activators[j]]=0;
                    
                    /*list the positions of all the binding sites of k*/
                    for(l=0;l<20;l++)
                        pos_binding_sites_of_k[l]=-200;
                    N_binding_sites_of_k=0;
                    for(k=j+1;k<N_activators;k++)
                    {
                        for(l=0;l<20;l++)
                            pos_binding_sites_of_k[l]=-1;
                        N_binding_sites_of_k=0;
                        gene_id=genotype->cisreg_cluster[i][0];                     
                        for(site_id=0;site_id<genotype->binding_sites_num[gene_id];site_id++)
                        {
                ...
                /*******************************count c1-ffls ************************/
                    ...
                    {
                        genotype->N_motifs[0]+=cluster_size*N_copies_reg_by_env;
                    ...
                    for(k=0;k<N_copies_not_reg_by_env;k++)
                    {
                        ...
                        if(!found_bs) // j is not regulated by k  
                        {
                            ...
                            {    
                                ...
                                if(!hindrance[genotype->which_protein[j]][genotype->which_protein[k]])
                    ...
                    for(k=j+1;k<N_copies_reg_by_env;k++)
                    {
                        ...
                        {                    
                            ...
                            if(!found_bs) // j is not regulated by k
                            {
                                ...
                                else // if k is not regulated by j either, then we have a parallel structure
                                {
        ...
        #if DIRECT_REG    
            genotype->proportion_motifs[0]=(float)genotype->N_motifs[0]/(genotype->N_act_genes*genotype->protein_pool[genotype->nproteins-1][0][0]);
            genotype->proportion_motifs[1]=(float)genotype->N_motifs[1]/(genotype->N_act_genes*genotype->protein_pool[genotype->nproteins-1][0][0]);
        #endif
        ...
        N_all_motifs=genotype->protein_pool[genotype->nproteins-1][0][0]*(genotype->N_act_genes_reg_by_env*genotype->N_act_genes_not_reg_by_env+genotype->N_act_genes_reg_by_env*(genotype->N_act_genes_reg_by_env-1)/2);
        genotype->proportion_motifs[2]=(float)genotype->N_motifs[2]/N_all_motifs;
        genotype->proportion_motifs[3]=(float)genotype->N_motifs[3]/N_all_motifs;
        genotype->proportion_motifs[4]=(float)genotype->N_motifs[4]/N_all_motifs;
        genotype->proportion_motifs[5]=(float)genotype->N_motifs[5]/N_all_motifs;
        genotype->proportion_motifs[6]=(float)genotype->N_motifs[6]/N_all_motifs;
        genotype->proportion_motifs[7]=(float)genotype->N_motifs[7]/N_all_motifs; 
    }    
    else
    {
        for(i=0;i<8;i++)
...
void tidy_output_files(char *file_genotype_summary, char *file_mutations)
{
    ...
    fclose(fp1);
    fclose(fp2);
    remove("proportion_c1ffl");
    rename("temp","proportion_c1ffl");
