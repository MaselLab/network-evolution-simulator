...
const float MEAN_TRANSLATION_INIT_RATE=0.436;
const float SD_TRANSLATION_INIT_RATE=0.408;
...
const float DEFAULT_MINIMAL_UPDATE_INTERVAL=0.1; /*min*/
float minimal_update_interval;
const float SIGNIFICANT_CHANGE_IN_PROBABILITY_OF_BINDING=0.01;
const float MAX_CHANGE_IN_PROBABILITY_OF_BINDING=0.05; 
...
const float sd_mutation_effect=0.78; 
float miu_ACT_TO_INT_RATE=2.27;
float miu_mRNA_decay=-0.49;
float miu_protein_decay=-2.83;
float miu_translation_init=-0.564;
float miu_Kd=-5.0;
                        ...
                        float tdevelopment)
{
    ...
    state->t_to_update_probability_of_binding=0.0;
                    ...
                    int thread_id)
{
    ...
    float diff_PA,diff_PR,diff_PnotAnoR,diff_PAnoR;  
    /* reset rates */
...
//    error_type=0;   
    ...
    {  
        if(UPDATE_WHAT==CHANGE_SIGNAL)
        {
            interval_to_update_probability_of_binding=minimal_update_interval;
        }
        else if(UPDATE_WHAT!=DO_NOTHING)
        {
            if(UPDATE_WHAT-NGENES>0) //mRNA finishes loading ribosomes 
                interval_to_update_probability_of_binding=genotype->update_interval[UPDATE_WHAT-NGENES][0]; 
            else   //mRNA degraded  
                interval_to_update_probability_of_binding=genotype->update_interval[UPDATE_WHAT][1];  
        }
        else //DO_NOTHING
        {
            too_much_error=0;        
            ...
            {
                diff_PA=fabs(state->P_A[i]-state->last_P_A[i]);
//                if(UPDATE_WHAT!=CHANGE_SIGNAL)
//                {
//                    max_change[0]=(max_change[0]>diff_PA)?max_change[0]:diff_PA;
//                    max_change[1]=(max_change[1]>diff_PR)?max_change[1]:diff_PR;
//                    max_change[2]=(max_change[2]>diff_PAnoR)?max_change[2]:diff_PAnoR;
//                    max_change[3]=(max_change[3]>diff_PnotAnoR)?max_change[3]:diff_PnotAnoR;
//                }
                if(diff_PA>SIGNIFICANT_CHANGE_IN_PROBABILITY_OF_BINDING)
                {
                    too_much_error=1;                
//                    if(diff_PA>MAX_CHANGE_IN_PROBABILITY_OF_BINDING)
//                        error_type=1; 
                    break;                        
                }              
                diff_PAnoR=fabs(state->P_A_no_R[i]-state->last_P_A_no_R[i]);
                if(diff_PAnoR>SIGNIFICANT_CHANGE_IN_PROBABILITY_OF_BINDING)
                {
                    too_much_error=1; 
//                    if(diff_PAnoR>MAX_CHANGE_IN_PROBABILITY_OF_BINDING)
//                        error_type=1;                    
                    break;
                }
                diff_PnotAnoR=fabs(state->P_NotA_no_R[i]-state->last_P_NotA_no_R[i]);                
                if(diff_PnotAnoR>SIGNIFICANT_CHANGE_IN_PROBABILITY_OF_BINDING)
                {
                    too_much_error=1; 
//                    if(diff_PnotAnoR>MAX_CHANGE_IN_PROBABILITY_OF_BINDING)
//                        error_type=1;                                         
                    break;
                }
                diff_PR=fabs(state->P_R[i]-state->last_P_R[i]);
                if(diff_PR>SIGNIFICANT_CHANGE_IN_PROBABILITY_OF_BINDING)
                {
                    too_much_error=1; 
//                    if(diff_PR>MAX_CHANGE_IN_PROBABILITY_OF_BINDING)
//                        error_type=1;                                           
                    break;                        
                } 
            }
            if(!too_much_error)
                interval_to_update_probability_of_binding=(t-state->last_event_t)*2.0;//the change in Pact is moderate, double the update interval     
            else        
                interval_to_update_probability_of_binding=(t-state->last_event_t)*0.5;//otherwise, reduce it by a half        
            if(interval_to_update_probability_of_binding<minimal_update_interval)
                interval_to_update_probability_of_binding=minimal_update_interval;
        }           
            
        /*Update the next time that Pact will be updated mandatorily*/
        t_to_update_probability_of_binding=t+interval_to_update_probability_of_binding;
...
//    {
//        N_update[1]++;
//        if(error_type==1 && UPDATE_WHAT!=CHANGE_SIGNAL)
//        {
//            N_update[2]++;   
//        }
//        
                                        ...
                                        char *effect_of_effector)
{
    ...
    *dt = state->mRNA_transl_init_time_end_head->time - t;         
    ...
    if(genotype->which_protein[gene_id]==genotype->nproteins-1)//if the mRNA encodes a non-sensor TF, there could be a huge change in TF concentration
        ...
        return gene_id+NGENES; //so update Pact and reset updating interval to MIN_INTERVAL_TO_UPDATE_PACT
}
 ...
 */
float calc_tprime(Genotype *genotype, CellState* state, float *number_of_selection_protein_bf_dt, float dt, float given_amount) 
{
    int n_copies;
    int i;          
    ...
    {
        protein_decay_rate[i]=genotype->protein_decay_rate[genotype->protein_pool[genotype->nproteins-1][1][i]];
        protein_synthesis_rate[i]=state->protein_synthesis_index[genotype->protein_pool[genotype->nproteins-1][1][i]]*protein_decay_rate[i];
    }       
...
void clone_genotype(Genotype *genotype_templet, Genotype *genotype_clone)
{
    ...
    for(i=0;i<NGENES;i++)
    {
        ...
        genotype_clone->update_interval[i][0]=TIME_INFINITY;
        genotype_clone->update_interval[i][1]=TIME_INFINITY;
    }    
    ...
    {            
        ...
        genotype_clone->update_interval[i][0]=genotype_templet->update_interval[i][0];
        genotype_clone->update_interval[i][1]=genotype_templet->update_interval[i][0];
    } 
    /* copy TF information*/
    for(i=0;i<NPROTEINS;i++)
    {
...
{    
    ...
    {           
        ...
        *t += dt;  /* advance time by the dt */        
        ...
        float dt_copy,x_copy,rate_total_Gillespie_rate_copy,t_copy;  
        int event2;           
        fixed_time=*t+dt;
        ...
        dt_copy=dt;
        x_copy=x; 
        rate_total_Gillespie_rate_copy=rates->total_Gillespie_rate; 
#endif        
        x -= dt*rates->total_Gillespie_rate;  /* we've been running with rates->total_Gillespie_rate for dt, so substract it from x*/        
        calc_all_rates(genotype, state, rates, *t, UPDATE_WHAT,thread_id);  /* update rates->total_Gillespie_rate and re-compute a new dt */      
        dt = x/rates->total_Gillespie_rate;
...
{     
    ...
    switch (event) 
    {
        ...
        case 6: /* mandatorily updating Pact and Prep*/
            *dt=state->t_to_update_probability_of_binding-t;
            update_protein_number_and_fitness(genotype, state, rates, *dt, t, *effect_of_effector, end_state, mut_step, mut_record);          
            break;
...
//        int thread_ID=0;
        ...
        minimal_update_interval=DEFAULT_MINIMAL_UPDATE_INTERVAL;   
        determine_minimal_update_interval(genotype,&minimal_update_interval,SIGNIFICANT_CHANGE_IN_PROBABILITY_OF_BINDING, KD2APP_KD,env1_t_development);
...
{    
    ...
    {           
        ...
        *t += dt;                     
        ...
        calc_all_rates(genotype, state, rates, *t, UPDATE_WHAT,thread_id,N_update,max_change); 
        dt = x/rates->total_Gillespie_rate;	
        if(dt<0.0)	
            dt=TIME_OFFSET;         
        /* check to see there aren't more fixed events to do */
...
{     
    ...
    minimal_update_interval=DEFAULT_MINIMAL_UPDATE_INTERVAL;   
    determine_minimal_update_interval(genotype,&minimal_update_interval,MAX_CHANGE_IN_PROBABILITY_OF_BINDING, KD2APP_KD);
//    #pragma omp parallel num_threads(N_THREADS)
    {
//        int thread_ID=omp_get_thread_num();
        int thread_ID=0;
...
//        #pragma omp critical
        ...
        {             
            ...
            t = 0.0;
            calc_all_rates(&genotype_clone, &state_clone, &rate_clone, t, INITIALIZATION, thread_ID,N_updateA[thread_ID*N_replicates_per_thread+i],max_changeA[thread_ID*N_replicates_per_thread+i]);
            timepoint=0;
            while(t<env1_t_development && end_state==1)
            {    
        ...
        {              
            ...
            t = 0.0;
            calc_all_rates(&genotype_clone, &state_clone, &rate_clone, t, INITIALIZATION,thread_ID,N_updateB[thread_ID*N_replicates_per_thread+i],max_changeB[thread_ID*N_replicates_per_thread+i]);        
            timepoint=0;
            while(t<env2_t_development && end_state==1)
            {
                do_single_timestep_plotting(&genotype_clone, 
...
void mut_kinetic_constant(Genotype *genotype, Mutation *mut_record, RngStream RS)
{
    ...
    {  
        genotype->active_to_intermediate_rate[which_gene]=mut_make_new_value(genotype->active_to_intermediate_rate[which_gene],miu_ACT_TO_INT_RATE,RS);
        /*record mutation info*/
    ...
    else /******************************** mut mRNAdecay **********************/
    {
        ...
        {       
            genotype->mRNA_decay_rate[which_gene]=mut_make_new_value(genotype->mRNA_decay_rate[which_gene],miu_mRNA_decay,RS);
            /*record mutation info*/
        ...
        else /*************************** mut translation *********************/
        {
            ...
            {       
                genotype->translation_rate[which_gene]=mut_make_new_value(genotype->translation_rate[which_gene],miu_translation_init,RS);        
            ...
            {  
                ...
                {                   
                    genotype->protein_decay_rate[which_gene]=mut_make_new_value(genotype->protein_decay_rate[which_gene],miu_protein_decay,RS);                  
...
float mut_make_new_value(float old_val, float miu, RngStream RS)
{
    ...
    while(new_val==old_val)    
        new_val=old_val*pow(10.0,sd_mutation_effect*gasdev(RS)+mutational_regression_rate*(miu-log(old_val)));
    return new_val;
}
...
void initialize_cache(Genotype *genotype)
{
    ...
    for(j=0;j<NGENES;j++)
    {
        ...
        genotype->update_interval[j][0]=TIME_INFINITY;
        genotype->update_interval[j][1]=TIME_INFINITY;
        genotype->Kd[j]=-1.0;
        for(k=0;k<NGENES;k++)        
...
#else          
    #if FORCE_SINGLE_FFL
            if(genotype_ori->N_motifs[23]!=0 && genotype_ori->N_motifs[9]==0 && genotype_ori->N_motifs[23]==genotype_ori->N_motifs[18]) //force single ffl
                        ...
                        RngStream RS_parallel[N_THREADS])
{
    ...
    N_EFFECTOR_GENES=EFFECTOR_GENES;
    N_TF_GENES=TFGENES;
    miu_ACT_TO_INT_RATE=2.27;
    miu_mRNA_decay=-0.49;       
    miu_translation_init=-0.564; 
...
void print_core_c1ffls(Genotype *genotype)
{
    ...
    fprintf(fp,"%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
                                                                                                genotype->N_motifs[0],
                                                                                                genotype->N_motifs[1],
                                                                                                genotype->N_motifs[2],
                                                                                                genotype->N_motifs[3],
                                                                                                ...
                                                                                                genotype->normalizer3,
                                                                                                genotype->normalizer4);
        fclose(fp); 
}
...
void find_ffl(Genotype *genotype)
{
    ...
    int N_motifs;
    int N_copies_reg_by_env_per_protein, N_copies_not_reg_by_env_per_protein;
    int N2_copies_reg_by_env,N2_copies_not_reg_by_env;
    ...
    {        
        ...
        {              
            ...
            {                 
                ...
                for(j=0;j<N_copies_reg_by_env;j++)
                {
                    ...
                    {
                        /*j and k must encode different TFs*/
                        if(genotype->which_protein[copies_reg_by_env[j]]!=genotype->which_protein[copies_not_reg_by_env[k]])
                        {
                            /*search bs of k on j*/
                ...
                for(j=0;j<N_copies_reg_by_env;j++)
                {
                    for(k=j+1;k<N_copies_reg_by_env;k++)
                    {
                        ...
                        if(k!=j && genotype->which_protein[copies_reg_by_env[j]]!=genotype->which_protein[copies_reg_by_env[k]])
                        {                    
                            /*search bs of k on j*/
                            gene_id=copies_reg_by_env[j];
                            found_bs=0;
        ...
        /*calculate normalizers of the numbers of motifs*/
#if DIRECT_REG
        genotype->normalizer1=genotype->N_act_genes;  
        genotype->normalizer2=genotype->N_act_genes;
        genotype->normalizer3=genotype->N_act_genes;
        genotype->normalizer4=genotype->N_act_genes;
#else
        genotype->normalizer1=0;
        genotype->normalizer2=0;
        genotype->normalizer3=0;
        genotype->normalizer4=0;
        N_copies_reg_by_env=0;
        N_copies_not_reg_by_env=0;
        N2_copies_reg_by_env=0;
        N2_copies_not_reg_by_env=0;
        for(i=N_SIGNAL_TF;i<genotype->nproteins-1;i++)
        {            
            if(genotype->protein_identity[i]==ACTIVATOR)
            {
                N_copies_reg_by_env_per_protein=0;
                N_copies_not_reg_by_env_per_protein=0;
                for(j=0;j<genotype->protein_pool[i][0][0];j++)
                {
                    gene_id=genotype->protein_pool[i][1][j];
                    found_bs=0;
                    for(site_id=0;site_id<genotype->binding_sites_num[gene_id];site_id++)
                    {
                        if(genotype->all_binding_sites[gene_id][site_id].tf_id==N_SIGNAL_TF-1)
                    ...
                    if(found_bs)
                        N_copies_reg_by_env_per_protein++;
                    else
                        N_copies_not_reg_by_env_per_protein++;
                }
                if(N_copies_reg_by_env_per_protein>N_copies_not_reg_by_env_per_protein)
                {
                    genotype->normalizer1-=genotype->protein_pool[i][0][0]*(genotype->protein_pool[i][0][0]-1)/2;
                    N_copies_reg_by_env+=genotype->protein_pool[i][0][0];
                    genotype->normalizer2-=genotype->protein_pool[i][0][0]*(genotype->protein_pool[i][0][0]-1)/2;
                    N2_copies_reg_by_env+=genotype->protein_pool[i][0][0];
                }
                else if(N_copies_reg_by_env_per_protein==N_copies_not_reg_by_env_per_protein)
                {
                    genotype->normalizer1-=genotype->protein_pool[i][0][0]*(genotype->protein_pool[i][0][0]-1)/2;
                    N_copies_reg_by_env+=genotype->protein_pool[i][0][0];                    
                    N2_copies_not_reg_by_env+=genotype->protein_pool[i][0][0];
                }
                else
                {
                    N_copies_not_reg_by_env+=genotype->protein_pool[i][0][0];
                    N2_copies_not_reg_by_env+=genotype->protein_pool[i][0][0];
                }
            }
        }  
        genotype->normalizer3=genotype->normalizer1+N_copies_reg_by_env*(N_copies_reg_by_env-1)/2;
        genotype->normalizer1+=N_copies_reg_by_env*N_copies_not_reg_by_env+N_copies_reg_by_env*(N_copies_reg_by_env-1)/2;
        genotype->normalizer2+=N2_copies_reg_by_env*N2_copies_not_reg_by_env+N2_copies_reg_by_env*(N2_copies_reg_by_env-1)/2;
        genotype->normalizer4=N_copies_reg_by_env*N_copies_not_reg_by_env;
#endif
        genotype->normalizer1*=genotype->protein_pool[genotype->nproteins-1][0][0];
        genotype->normalizer2*=genotype->protein_pool[genotype->nproteins-1][0][0];
        genotype->normalizer3*=genotype->protein_pool[genotype->nproteins-1][0][0];
        genotype->normalizer4*=genotype->protein_pool[genotype->nproteins-1][0][0];
    }    
