...
float tdevelopment = 70.0;/* default  development time: can be changed at runtime */
...
int output = 0;
long seed = 150; //28121 94571        something is wrong here: changing seed changes nothing
...
void initialize_sequence(char *Seq, int len, int ploidy, int num_elements) {
	float x;
	int i;
	...
	for (i = 0; i < len / ploidy; i++) {
		first = (i / current_element) * ploidy * current_element + i
				% current_element;
...
void print_all_binding_sites(int copies[NGENES],
		AllTFBindingSites *all_binding_sites, int numElements,
		...
		while (i >= NGENES || j < copies[i]) {
			if (i < TFGENES)
				printf("TF sequence gene %2d (copy %d): %.*s\n", i, j,
						TF_ELEMENT_LEN, tf_seq[i][j]);
			else
				printf("            gene %2d (copy %d) does not encode a TF\n",
						i, j);
			if (i < NGENES) {
				printf("cis-reg     gene %2d (copy %d): %.*s\n", i, j,
						CISREG_LEN, cisreg_seq[i][j]);
				printf("ID range    gene %2d (copy %d): [%3d, %3d]\n", i, j,
						site_id_pos[i][j][0], site_id_pos[i][j][1]);
				printf("\n");
			} else {
				printf(
						"            gene %2d (copy %d): no cis-reg gene here\n",
						i, j);
				printf("\n");
	...
	for (i = 0; i < numElements; i++) {
		...
		printf(
				" (sequence %.*s)\n",
				CISREG_LEN,
				cisreg_seq[all_binding_sites[i].cisreg_id][all_binding_sites[i].gene_copy]);
		printf(" transcription-factor: %3d", all_binding_sites[i].tf_id);
		printf(
				" (sequence: %.*s)\n",
				TF_ELEMENT_LEN,
				tf_seq[all_binding_sites[i].tf_id][all_binding_sites[i].gene_copy]);
		printf("  L-edge of %2dbp hind: %3d\n", HIND_LENGTH,
				all_binding_sites[i].left_edge_pos);
...
void initialize_genotype_fixed(Genotype *genotype, float kdis[],
		int genotype_id) {
	int i, j, p;
	...
	for (p = 0; p < TFGENES; p++) {
		if (HIND_LENGTH == TF_ELEMENT_LEN) {
			...
			genotype->hindrance_positions[p] = rint(ran1(&seed) * (HIND_LENGTH
					- TF_ELEMENT_LEN));
		}
	}
	...
	for (i = 0; i < NGENES; i++) {
		...
		while (genotype->proteindecay[i] < 0.0) {
			if (ran1(&seed) < 0.08421)
				...
				genotype->proteindecay[i]
						= exp(0.7874 * gasdev(&seed) - 3.7665);
		}
 ...
 */
void initialize_genotype(Genotype *genotype, Genotype *clone, float kdis[],
		int genotype_id) {
	int p;

	initialize_sequence((char *) genotype->cisreg_seq, CISREG_LEN * MAX_COPIES
			*NGENES, MAX_COPIES, NGENES);
	...
	if (genotype_id == 0) {
		initialize_sequence((char *) genotype->tf_seq, TF_ELEMENT_LEN
				* MAX_COPIES * TFGENES, MAX_COPIES, TFGENES);
		initialize_genotype_fixed(genotype, kdis, genotype_id);
		// otherwise clone it from the first instance
	} else {
		/* copy the TF sequence data */
		memcpy(genotype->tf_seq, clone->tf_seq,
	...
	calc_all_binding_sites(genotype->copies, genotype->cisreg_seq,
			genotype->tf_seq, &(genotype->binding_sites_num),
			&(genotype->all_binding_sites), genotype->hindrance_positions,
 ...
 */
void mutate(Genotype *gene, int gene_id, int gene_copy, float m) {
	int k, p;
	char x;
	float q;
 ...
 */
//DESTROY??
int calc_all_binding_sites_copy(
		char cisreg_seq[NGENES][MAX_COPIES][CISREG_LEN],
		char tf_seq[TFGENES][MAX_COPIES][TF_ELEMENT_LEN],
		int binding_sites_num, AllTFBindingSites **all_binding_sites,
		int *max_alloc, int gene_id, int gene_copy, int hind_pos[TFGENES]) {
	...
	for (i = 0; i < CISREG_LEN - TF_ELEMENT_LEN; i++) { /* scan forwards */
		for (tfind = 0; tfind < TFGENES; tfind++) { /* only loop through TF genes */
			...
			for (j = i; j < i + TF_ELEMENT_LEN; j++) {
				if (cisreg_seq[gene_id][gene_copy][j]
						== tf_seq[tfind][gene_copy][j - i])
					match++;
			}
			if (match >= NMIN) {
				if (binding_sites_num + 1 >= max_binding_site_alloc) {
					max_binding_site_alloc = 2 * max_binding_site_alloc;
					*all_binding_sites = realloc(*all_binding_sites,
							max_binding_site_alloc * sizeof(AllTFBindingSites));
				...
				if (((i - hind_pos[tfind]) >= 0) && (((i - hind_pos[tfind])
						+ (HIND_LENGTH - 1)) < CISREG_LEN)) {
					(*all_binding_sites)[binding_sites_num].cisreg_id = gene_id;
					(*all_binding_sites)[binding_sites_num].gene_copy
							= gene_copy;
					(*all_binding_sites)[binding_sites_num].tf_id = tfind;
					(*all_binding_sites)[binding_sites_num].strand = 0;
					(*all_binding_sites)[binding_sites_num].hamming_dist
							= TF_ELEMENT_LEN - match;
					(*all_binding_sites)[binding_sites_num].hind_pos
							= hind_pos[tfind];
					(*all_binding_sites)[binding_sites_num].left_edge_pos = i
							- hind_pos[tfind];
					binding_sites_num++;
				}
			}
	...
	for (i = CISREG_LEN - 1; i >= TF_ELEMENT_LEN - 1; i--) { /* scan backwards */
		for (tfind = 0; tfind < TFGENES; tfind++) { /* only loop through TF genes */
			match = 0;
			for (j = i; j > i - TF_ELEMENT_LEN; j--)
				if ((cisreg_seq[gene_id][gene_copy][j] == 'a'
						&& tf_seq[tfind][gene_copy][i - j] == 't')
						|| (cisreg_seq[gene_id][gene_copy][j] == 't'
								&& tf_seq[tfind][gene_copy][i - j] == 'a')
						|| (cisreg_seq[gene_id][gene_copy][j] == 'c'
								&& tf_seq[tfind][gene_copy][i - j] == 'g')
						|| (cisreg_seq[gene_id][gene_copy][j] == 'g'
								&& tf_seq[tfind][gene_copy][i - j] == 'c'))
					match++;
			if (match >= NMIN) {
				if (binding_sites_num + 1 >= max_binding_site_alloc) {
					max_binding_site_alloc = 2 * max_binding_site_alloc;
				...
				if (((i - TF_ELEMENT_LEN + 1 - hind_pos[tfind]) >= 0) && (((i
						- TF_ELEMENT_LEN + 1 - hind_pos[tfind]) + (HIND_LENGTH
						- 1)) < CISREG_LEN)) {
					(*all_binding_sites)[binding_sites_num].cisreg_id = gene_id;
					(*all_binding_sites)[binding_sites_num].gene_copy
							= gene_copy;
					...
					(*all_binding_sites)[binding_sites_num].hind_pos
							...
							= hind_pos[tfind];
					(*all_binding_sites)[binding_sites_num].left_edge_pos = i
							- TF_ELEMENT_LEN + 1 - hind_pos[tfind];
					binding_sites_num++;
				}
			}
		}
...
void calc_all_binding_sites(int copies[NGENES],
		char cisreg_seq[NGENES][MAX_COPIES][CISREG_LEN],
		char tf_seq[TFGENES][MAX_COPIES][TF_ELEMENT_LEN],
		int *new_binding_sites_num, AllTFBindingSites **all_binding_sites,
		int hind_pos[TFGENES], int sites_per_gene[NGENES],
		int site_id_pos[NGENES][MAX_COPIES][2]) {
	int p, max_binding_site_alloc, binding_sites_num;
	int gene_id;

	max_binding_site_alloc = MAXELEMENTS;
	*all_binding_sites = malloc(max_binding_site_alloc
			* sizeof(AllTFBindingSites));
	if (!(*all_binding_sites)) {
		exit(1);
	}
	binding_sites_num = 0;
		...
		for (p = 0; p < MAX_COPIES; p++) { /* loop through the maximum copies possible */
			if (p < copies[gene_id]) {
				...
				binding_sites_num = calc_all_binding_sites_copy(cisreg_seq,
						tf_seq, binding_sites_num, all_binding_sites,
						&max_binding_site_alloc, gene_id, p, hind_pos);
	...
	*all_binding_sites = realloc(*all_binding_sites, binding_sites_num
			* sizeof(AllTFBindingSites));
	if (!(*all_binding_sites)) {
		exit(1);
	}
...
int add_fixed_event(int i, int p, float t, FixedEvent **start,
		FixedEvent **last) {
	FixedEvent *newtime;
	int pos;
...
void add_time_point(float time, float conc, TimeCourse **start,
		TimeCourse **last) {
	TimeCourse *newtime;
...
void add_fixed_event_end(int gene_id, int gene_copy, float t,
		FixedEvent **start, FixedEvent **last) {
	FixedEvent *newtime;
...
void delete_fixed_event(int gene_id, int gene_copy, int i, FixedEvent **start,
		FixedEvent **last) {
...
void delete_fixed_event_start(FixedEvent **start, FixedEvent **last) {
	FixedEvent *info;
 ...
 */
void initialize_cell(CellState *state,Genotype *genotype, int cell_id,
		int copies[NGENES], float mRNAdecay[NGENES], float meanmRNA[NGENES],
		float init_protein_conc[NPROTEINS], int burn_in) {
	int i, j, k, totalmRNA;
	float t;

	/* initialize the ID of the cell */
	genotype->cell_id = cell_id;

	/* initialize the founder cell using the current ID */
	genotype->founder_id = cell_id;

	/* don't start in S phase */
	state->in_s_phase = 0;

	/* initialize whether to do kon burn-in or not */
	genotype->burn_in = burn_in;

	/* start cell size at 0.5 */
	state->cell_size = 0.5;

	/* start cell with no divisions */
	genotype->divisions = 0;
	...
	for (i = 0; i < NGENES; i++) {
		...
		state->mRNA_nuclear_num[i] = (int) bnldev(STARTNUCLEUS, totalmRNA,
				&seed);
		...
		for (k = 0; k < state->mRNA_cyto_num[i]; k++) {
			...
			if (t < TTRANSLATION) {
				...
				add_fixed_event(i, -1, TTRANSLATION - t,
						&(state->mRNA_transl_time_end),
						&(state->mRNA_transl_time_end_last));
			}
		}

		int total_mRNA_transcribing = (int) poidev(meanmRNA[i] * TTRANSCRIPTION
				* mRNAdecay[i], &seed);
		...
		for (j = 0; j < MAX_COPIES; j++) {
			if (j < current_ploidy) {
				state->mRNA_transcr_num[i][j] = (j == 0) ? mRNA_copy1
						: mRNA_copy2;
				for (k = 0; k < state->mRNA_transcr_num[i][j]; k++)
					add_fixed_event(i, j, ran1(&seed) * TTRANSCRIPTION,
							&(state->mRNA_transcr_time_end),
							&(state->mRNA_transcr_time_end_last));
			} else {
 ...
 */
//DESTROY?
void calc_from_state(Genotype *genotype, CellState *state, float transport[],
		float mRNAdecay[]) {
	int i, j, k;
	float protein_conc_for_tfid;
	float protein_decay;
	//QUESTION
	for (i = 0; i < NPROTEINS; i++) {
		/* if protein decay is otherwise going to fall below cut-off, use aging term */
		protein_decay
				= genotype->proteindecay[i] >= protein_aging ? genotype->proteindecay[i]
	...
	state->transport = 0.0;
	state->transport_operations = 0;
	state->mRNAdecay = 0.0;
	state->mRNAdecay_operations = 0;
	state->pic_disassembly = 0.0;
	state->pic_disassembly_operations = 0;
	...
	for (i = 0; i < NGENES; i++) {
		...
		transport[i] = TRANSPORTATIONRATE * (float) (state->mRNA_nuclear_num[i]);
		state->transport += transport[i];
		state->transport_operations++;
	}
	...
	for (j = 0; j < MAX_COPIES; j++) {
		...
		for (i = 0; i < NGENES; i++) {
			if (genotype->copies[i] > j) {
				state->state_change_ids[STATE_ZERO][j][pos] = i;
				pos++;
			}
		}
	}
	...
	for (j = 0; j < MAX_COPIES; j++) {
		state->acetylation_num[j] = 0;
		state->deacetylation_num[j] = 0;
		state->pic_assembly_num[j] = 0;
		state->transcript_init_num[j] = 0;
		state->pic_disassembly_num[j] = 0;
	}

	/* now set the per copy acetylation rate  */
	for (i = 0; i < NGENES; i++) {
		for (j = 0; j < genotype->copies[i]; j++) {
			state->acetylation_num[j]++;
		}
	}

	//do not execute
	if (0) //VERBOSE JU CHANGE
		for (j = 0; j < MAX_COPIES; j++) {
			LOG_VERBOSE("state->acetylation_num[%d]=%d\n", j, state->acetylation_num[j]);
			LOG_ERROR("state->acetylation_num[%d]=%d\n", j, state->acetylation_num[j]);
...
int does_fixed_event_end(FixedEvent *mRNA_transl_time_end,
		FixedEvent *mRNA_transcr_time_end, FixedEvent *replication_time_end,
		float t) {
	int retval;
	float t1;
	float t2;
	...
	if (mRNA_transcr_time_end == NULL && mRNA_transl_time_end == NULL
			&& replication_time_end == NULL) {
		retval = 0;
	} else {
		// TODO: rewrite this to avoid use of magic number
		t1 = mRNA_transcr_time_end ? mRNA_transcr_time_end->time
				: TIME_INFINITY;
		...
		if ((t1 < t2) && (t1 < t3) && (t1 < t)) {
			if (mRNA_transcr_time_end == NULL)
				retval = 0;
			else
				retval = 1;
		} else if ((t2 < t3) && (t2 < t1) && (t2 < t)) {
			if (mRNA_transl_time_end == NULL)
				retval = 0;
			else
				retval = 2;
		} else if ((t3 < t1) && (t3 < t2) && (t3 < t)) {
			if (replication_time_end == NULL)
				retval = 0;
			else
				retval = 3;
		} else {
 ...
 */
void end_transcription(float *dt, float t, CellState *state,
	...
	*dt = state->mRNA_transcr_time_end->time - t;
	...
	delete_fixed_event_start(&(state->mRNA_transcr_time_end),
			&(state->mRNA_transcr_time_end_last));

	/* add rate TRANSPORTATIONRATE to transport and Gillespie rates */
	transport[i] += TRANSPORTATIONRATE;
	state->transport += TRANSPORTATIONRATE;
	state->transport_operations++;
	state->transcript_init_num[i] = 0;

	//TODO: Fix this logging message
	//LOG_VERBOSE("add one new mRNA in nucleus, updating transport[%d]=%g, state->transport=%g\n", i, transport[i], state->transport);
 ...
 */
void disassemble_PIC(CellState *state, Genotype *genotype, int gene_id,
		int gene_copy) {
	float disassembly = genotype->pic_disassembly[gene_id][gene_copy];
	remove_from_array(gene_id, STATE_TWO,
			state->state_change_ids[STATE_TWO][gene_copy],
			&(state->transcript_init_num[gene_copy]), (int) 1);
	remove_from_array(gene_id, STATE_ONE,
			state->state_change_ids[STATE_ONE][gene_copy],
			&(state->pic_disassembly_num[gene_copy]), (int) 1);
	state->pic_disassembly -= disassembly;
	state->pic_disassembly_operations++;

	/* disassemble PIC in OFF state */
	if (state->active[gene_id][gene_copy] == 3) {
		(state->active[gene_id][gene_copy]) = 2;
		state->state_change_ids[STATE_ZERO][gene_copy][state->deacetylation_num[gene_copy]]
				= gene_id;
		(state->deacetylation_num[gene_copy])++;
	}
 ...
 */
void add_time_points(float time, float protein_conc[NPROTEINS],
		TimeCourse **timecoursestart, TimeCourse **timecourselast) {
	int i;

	for (i = 0; i < NPROTEINS; i++)
		add_time_point(time, protein_conc[i], &(timecoursestart[i]),
				&(timecourselast[i]));
}

void add_integer_time_points(float time, int protein_conc[NPROTEINS],
		TimeCourse **timecoursestart, TimeCourse **timecourselast) {
	int i;

	for (i = 0; i < NPROTEINS; i++)
		add_time_point(time, (float) protein_conc[i], &(timecoursestart[i]),
				&(timecourselast[i]));
}
 ...
 */
void reach_s_phase(CellState *state, Genotype *genotype, float t) {
	int i;
	...
	if (time_s_phase + time_g2_phase > 0.0) {
		for (i = 0; i < NGENES; i++) {
			...
			add_fixed_event(i, -1, t + genotype->replication_time[i],
					...
					&(state->replication_time_end_last));
		}
	}
}
 ...
 */
float compute_integral(float alpha, float c, float gmax, float deltat,
		float s_mRNA, float P, float Pp, float ect, float ect1) {
	return 1.0 / (pow(c, 2) * Pp) * gmax * (-alpha * ect1 * s_mRNA + c * (P
			* ect1 + alpha * deltat * s_mRNA));
}
 ...
 */
float compute_growth_rate_dimer(float *integrated_growth_rate, float alpha,
		float s_mRNA, float all_alpha[NGENES], int all_s_mRNA[NGENES], float P,
		float P_next, float t, float deltat, float c, float ect, float ect1,
		int cell_id) {
	int i;
	...
	LOG_VERBOSE_NOCELLID("t=%g (in min) t+deltat=%g (in min)",
			t, t+deltat);
	...
	if (((P > Pp) && (P_next >= P)) || ((P_next > Pp) && (P >= P_next))) { /* P > Pp throughout */
		...
		*integrated_growth_rate = compute_integral(alpha, c, gmax, deltat,
				s_mRNA, P, Pp, ect, ect1);
	} else if ((Pp > P) && (P_next > P)) { /* P < Pp up until t' then P > Pp */
		deltatprime = compute_tprime(c, P, alpha, s_mRNA);
		deltatrest = deltat - deltatprime;
		*integrated_growth_rate = compute_integral(alpha, c, gmax, deltatprime,
				s_mRNA, P, Pp, ect, ect1);
		...
		*integrated_growth_rate += compute_integral(alpha, c, gmax, deltatrest,
				s_mRNA, P, Pp, ect, ect1);
	} else {
...
//system("PAUSE");
   ...
   //printf("\n salph = %f,  ect1 = %f,  ect = %f, protein_conc[i] = %f\n", salph, ect1, ect, protein_conc[i]);
    ...
    if (i == SELECTION_GENE) {  /* now find out the protein concentration at end of dt interval and compute growth rate */
      ...
      fprintf(fp_cellsize[genotype->cell_id], "%g %g\n", t, state->cell_size);
    }
 ...
 * copy and location
 */
void get_gene(int rate_array[MAX_COPIES], int pos, int *gene_loc,
		int *gene_copy) {
	int i = 0;
	int total_rate = 0;
 ...
 *
 */
void transport_event(CellState *state,
		Genotype *genotype, float transport[NGENES],
		TimeCourse **timecoursestart, TimeCourse **timecourselast, float dt,
		float t, float x) {
	int i, j;
	float temp_rate;
	...
	update_protein_conc_cell_size(state->protein_conc, state, genotype, dt,
			t, timecoursestart, timecourselast, state->protein_conc);
	...
	add_fixed_event_end(i, -1, endtime, &(state->mRNA_transl_time_end),
			&(state->mRNA_transl_time_end_last));

	transport[i] -= TRANSPORTATIONRATE; /* decrease transport frequency */

	/* if below a threshold, make zero */
	if (transport[i] < 0.1 * TRANSPORTATIONRATE)
		transport[i] = 0.0;
	...
	if (state->transport < 0.1 * TRANSPORTATIONRATE)
		state->transport = 0.0;
}

void mRNA_decay_event(CellState *state,
		Genotype *genotype, float *mRNAdecay, TimeCourse **timecoursestart,
		TimeCourse **timecourselast, float dt, float t, float x) {
	int i = -1, j;
	float temp_rate = 0.0;
	...
	x = ran1(&seed) * ((float) (state->mRNA_cyto_num[i]
			+ state->mRNA_transl_cyto_num[i]));
	...
	if (x < (float) state->mRNA_cyto_num[i]) {
		...
		delete_fixed_event(i, -1, (int) trunc(x),
				&(state->mRNA_transl_time_end),
				&(state->mRNA_transl_time_end_last));
...
void histone_acteylation_event(CellState *state,
		Genotype *genotype, TimeCourse **timecoursestart,
		TimeCourse **timecourselast, float dt, float t) {
	int gene_loc, gene_copy;
	float x = ran1(&seed) * ((float) sum_rate_counts(state->acetylation_num));

	/* choose a particular gene to change state */
	get_gene(state->acetylation_num, (int) trunc(x), &gene_loc, &gene_copy);
	int gene_id = state->state_change_ids[STATE_ZERO][gene_copy][gene_loc];

	if (state->active[gene_id][gene_copy] != 0) {
	...
	state->active[gene_id][gene_copy] = 2;
	remove_from_array(gene_id, STATE_ZERO,
			state->state_change_ids[STATE_ZERO][gene_copy],
			&(state->acetylation_num[gene_copy]), (int) 1);
	...
	state->state_change_ids[STATE_ONE][gene_copy][state->pic_assembly_num[gene_copy]]
			= gene_id;
	(state->pic_assembly_num[gene_copy])++;
	//}
}

void histone_deacteylation_event(CellState *state,
		Genotype *genotype, TimeCourse **timecoursestart,
		TimeCourse **timecourselast, float dt, float t) {
	int gene_copy, gene_loc;
	float x = ran1(&seed) * ((float) sum_rate_counts(state->deacetylation_num));

	/* choose a particular gene and copy to change state */
	get_gene(state->deacetylation_num, (int) trunc(x), &gene_loc, &gene_copy);
	int gene_id = state->state_change_ids[STATE_ZERO][gene_copy][gene_loc];

	if (state->active[gene_id][gene_copy] != 1) {
	...
	state->active[gene_id][gene_copy] = 0;
	remove_from_array(gene_id, STATE_ZERO,
			state->state_change_ids[STATE_ZERO][gene_copy],
			&(state->deacetylation_num[gene_copy]), (int) 1);

}

void assemble_PIC_event(CellState *state,
		Genotype *genotype,
		//KonStates *kon_states,
		TimeCourse **timecoursestart, TimeCourse **timecourselast, float dt,
		float t) {
	float x = ran1(&seed) * ((float) sum_rate_counts(state->pic_assembly_num));

	int gene_copy;
	int gene_loc;

	/* choose a particular gene and copy to change state */
	get_gene(state->pic_assembly_num, (int) trunc(x), &gene_loc, &gene_copy);
	int gene_id = state->state_change_ids[STATE_ONE][gene_copy][gene_loc];

	if (state->active[gene_id][gene_copy] != 2) {
	...
	state->active[gene_id][gene_copy] = 3;
	remove_from_array(gene_id, STATE_ONE,
			state->state_change_ids[STATE_ONE][gene_copy],
			&(state->pic_assembly_num[gene_copy]), (int) 1);
	state->state_change_ids[STATE_TWO][gene_copy][state->transcript_init_num[gene_copy]]
			= gene_id;
	(state->transcript_init_num[gene_copy])++;
	state->state_change_ids[STATE_ONE][gene_copy][state->pic_disassembly_num[gene_copy]]
			= gene_id;

	(state->pic_disassembly_num[gene_copy])++;

	state->pic_disassembly += genotype->pic_disassembly[gene_id][gene_copy];
	state->pic_disassembly_operations++;
}

void disassemble_PIC_event(CellState *state,
		Genotype *genotype,
		//KonStates *kon_states,
		TimeCourse **timecoursestart, TimeCourse **timecourselast, float dt,
		float t, float x) {
	...
	while (j < NGENES * current_ploidy && x > 0) {
		j++;
		get_gene(state->pic_disassembly_num, j, &gene_loc, &gene_copy);
		x
				-= genotype->pic_disassembly[state->state_change_ids[STATE_ONE][gene_copy][gene_loc]][gene_copy];
	}
	if (j == NGENES * current_ploidy) {
		LOG_ERROR("error in PIC disassembly\n");
	}
	/* now get the gene_id */
	gene_id = state->state_change_ids[STATE_ONE][gene_copy][gene_loc];
	/* do the disassembling */
	disassemble_PIC(state, genotype, gene_id, gene_copy);
}

void transcription_init_event(CellState *state,
		Genotype *genotype,
		//KonStates *kon_states,
		TimeCourse **timecoursestart, TimeCourse **timecourselast, float dt,
		float t, float x) {
	...
	get_gene(state->transcript_init_num, (int) trunc(x), &gene_loc, &gene_copy);
	gene_id = state->state_change_ids[STATE_TWO][gene_copy][gene_loc];
	LOG_NOCELLID("transcription event gene %d, copy %d  gene loc %d\n", gene_id, gene_copy, gene_loc);
 ...
 */
void replicate_gene(CellState *state, Genotype *genotype,
		int gene_id, float t) {
	int i, k, p;// l,
	...
	for (p = 0; p < current_ploidy; p++)
		if ((state->active[gene_id][p] == 3 || state->active[gene_id][p]
				== 3))
			disassemble_PIC(state, genotype, gene_id, p);
	...
	for (i = 0; i < current_ploidy; i++) {
		p = i + current_ploidy;
		state->state_change_ids[STATE_ZERO][p][state->acetylation_num[p]]
				= gene_id;
		/* update the counts for the acetylation */
		state->acetylation_num[p]++;
	}
}

//DESTROY
//CHANGE JU
void recalibrate_cell(CellState *state,
		Genotype *genotype, float mRNAdecay[NGENES], float transport[NGENES],
		float dt) {
	int i, j;
	...
	for (i = 0; i < NGENES; i++) {
		...
		mRNAdecay[i] = genotype->mRNAdecay[i]
				* ((float) state->mRNA_cyto_num[i]
						+ (float) state->mRNA_transl_cyto_num[i]);
		state->mRNAdecay += mRNAdecay[i];
		state->mRNAdecay_operations++;

	}
	///LOG_ERROR("mRNAdecay = %f\n", state->mRNAdecay);
	// recompute and cache the total rate in data structure
	state->total += state->transport;
	state->total += state->mRNAdecay;
	state->total += state->pic_disassembly;

	//convert the counts back into rates using the constants
	for (j = 0; j < MAX_COPIES; j++) {
		state->total += (float) state->acetylation_num[j] * ACETYLATE;
		state->total += (float) state->deacetylation_num[j] * DEACETYLATE;
		state->total += (float) state->pic_assembly_num[j] * PICASSEMBLY;
		state->total += (float) state->transcript_init_num[j]
				* TRANSCRIPTINIT;
	}
}
 ...
 */
void clone_queue(FixedEvent **start_orig, FixedEvent **last_orig,
		FixedEvent **start_clone, FixedEvent **last_clone) {
	while (*start_orig != NULL) {
		//add_fixed_event((*start_orig)->gene_id, (*start_orig)->time, start_clone, last_clone);
		add_fixed_event((*start_orig)->gene_id, (*start_orig)->copy,
				(*start_orig)->time, start_clone, last_clone);
 ...
 */
void split_mRNA(FixedEvent **start_clone, FixedEvent **last_clone,
		int count_clone[NGENES], FixedEvent **start_daughter,
		FixedEvent **last_daughter, int count_daughter[NGENES],
		FixedEvent **start_mother, FixedEvent **last_mother,
		int count_mother[NGENES], int i, float fraction) {
	...
	for (j = 0; j < count_clone[i]; j++) {
		if (*start_clone != NULL) {
			...
			if (ran1(&seed) <= fraction) { /* fraction of time move to daughter */
				...
				add_fixed_event(gene_id, copy, time, start_daughter,
						last_daughter);
 ...
 */
void clone_cell(Genotype *genotype_orig, CellState *state_orig,
		Genotype *genotype_clone,
		CellState *state_clone) {
	int i, p;// k,

	genotype_clone->founder_id = genotype_orig->founder_id;
	state_clone->in_s_phase = state_orig->in_s_phase;
	genotype_clone->burn_in = genotype_orig->burn_in;
	genotype_clone->divisions = genotype_orig->divisions;
	...
	memcpy(genotype_clone->cisreg_seq, genotype_orig->cisreg_seq,
			sizeof(char[NGENES][MAX_COPIES][CISREG_LEN]));

	/* clone all other aspects of genotype */
	for (i = 0; i < NGENES; i++) {
		for (p = 0; p < MAX_COPIES; p++) {
			genotype_clone->site_id_pos[i][p][0]
					= genotype_orig->site_id_pos[i][p][0];
			genotype_clone->site_id_pos[i][p][1]
					= genotype_orig->site_id_pos[i][p][1];
			genotype_clone->activating[i][p] = genotype_orig->activating[i][p];
			genotype_clone->pic_disassembly[i][p]
					= genotype_orig->pic_disassembly[i][p];
		}
		genotype_clone->sites_per_gene[i] = genotype_orig->sites_per_gene[i];
		genotype_clone->copies[i] = genotype_orig->copies[i];
		genotype_clone->replication_time[i]
				= genotype_orig->replication_time[i];
	...
	for (i = 0; i < TFGENES; i++) {
		genotype_clone->hindrance_positions[i]
				= genotype_orig->hindrance_positions[i];
	}
	...
	for (i = 0; i < NGENES; i++) {
		for (p = 0; p < MAX_COPIES; p++) {
			...
			state_clone->state_change_ids[STATE_ZERO][p][i]
					= state_orig->state_change_ids[STATE_ZERO][p][i];
			state_clone->state_change_ids[STATE_ZERO][p][i]
					= state_orig->state_change_ids[STATE_ZERO][p][i];
			state_clone->state_change_ids[STATE_ONE][p][i]
					= state_orig->state_change_ids[STATE_ONE][p][i];
			state_clone->state_change_ids[STATE_TWO][p][i]
					= state_orig->state_change_ids[STATE_TWO][p][i];
			state_clone->state_change_ids[STATE_TWO][p][i]
					= state_orig->state_change_ids[STATE_TWO][p][i];

			state_clone->acetylation_num[p] = state_orig->acetylation_num[p];
			state_clone->deacetylation_num[p]
					= state_orig->deacetylation_num[p];
			state_clone->pic_assembly_num[p] = state_orig->pic_assembly_num[p];
			state_clone->transcript_init_num[p]
					= state_orig->transcript_init_num[p];
			state_clone->pic_disassembly_num[p]
					= state_orig->pic_disassembly_num[p];
			state_clone->mRNA_transcr_num[i][p]
					= state_orig->mRNA_transcr_num[i][p];
		}
		...
		state_clone->mRNA_transl_cyto_num[i]
				= state_orig->mRNA_transl_cyto_num[i];
	}

	/* clone queue and empty original */
	clone_queue(&(state_orig->mRNA_transcr_time_end),
			&(state_orig->mRNA_transcr_time_end_last),
			&(state_clone->mRNA_transcr_time_end),
			&(state_clone->mRNA_transcr_time_end_last));
	clone_queue(&(state_orig->mRNA_transl_time_end),
			&(state_orig->mRNA_transl_time_end_last),
			&(state_clone->mRNA_transl_time_end),
			&(state_clone->mRNA_transl_time_end_last));
}
...
void initialize_new_cell_genotype(Genotype *genotype, Genotype *genotype_clone) {
	...
	/* initialize hindrance for all TFGENES */
	for (p = 0; p < TFGENES; p++) {
		genotype->hindrance_positions[p]
				= genotype_clone->hindrance_positions[p];
	}
...
void initialize_new_cell_state(CellState *state, CellState state_clone,
	...
	if (state->mRNA_transcr_time_end != NULL) {
		...
		printf("time=%g, gene_id=%d\n", state->mRNA_transcr_time_end->time,
				state->mRNA_transcr_time_end->gene_id);
	}

	if (state->replication_time_end != NULL) {
		printf("after delete_queues  replication_time_end is not NULL\n");
		printf("time=%g, gene_id=%d\n", state->replication_time_end->time,
				state->replication_time_end->gene_id);
	}
	...
	genotype->burn_in = 0; /* only do burn-in at beginning of runs */
	state->division_time = TIME_INFINITY; /* reset division time for this cell */
	genotype->divisions = genotype_clone.divisions; /* copy division counter */
	genotype->cell_size = state_clone.cell_size * fraction; /* reset cell size */
	...
	for (i = 0; i < MAX_COPIES; i++) {
		for (j = 0; j < NGENES; j++)
			state->state_change_ids[STATE_ZERO][i][j] = -1;
		state->acetylation_num[i] = 0;
		state->deacetylation_num[i] = 0;
		state->pic_assembly_num[i] = 0;
		state->transcript_init_num[i] = 0;
		state->pic_disassembly_num[i] = 0;
	}
}
#endif
...
void initialize_new_cell_state_change_ids(CellState *state,
		CellState state_clone, int type,
		int count[MAX_COPIES],
		int clone_count[MAX_COPIES], int copy1, int copy2, int gene_id) {
	int j;
	...
	for (j = 0; j < clone_count[copy2]; j++) {
		if (state_clone.state_change_ids[type][copy2][j] == gene_id) {
			state->state_change_ids[type][1][count[1]]
					= state_clone.state_change_ids[type][copy2][j];
...
void initialize_new_cell_gene(Genotype *genotype, Genotype genotype_clone,
		CellState *state, CellState *state_clone, int gene_id, int copy1, int copy2,
		int *lastpos) {
	int j, k;

	for (k = 0; k < CISREG_LEN; k++) {
		genotype->cisreg_seq[gene_id][0][k]
				= genotype_clone.cisreg_seq[gene_id][copy1][k];
		genotype->cisreg_seq[gene_id][1][k]
				= genotype_clone.cisreg_seq[gene_id][copy2][k];
		genotype->cisreg_seq[gene_id][2][k]
				= genotype_clone.cisreg_seq[gene_id][copy1][k];
		genotype->cisreg_seq[gene_id][3][k]
				= genotype_clone.cisreg_seq[gene_id][copy2][k];
	}
	...
	if (gene_id < TFGENES) {
		for (k = 0; k < TF_ELEMENT_LEN; k++) {
			genotype->tf_seq[gene_id][0][k]
					= genotype_clone.tf_seq[gene_id][copy1][k];
			genotype->tf_seq[gene_id][1][k]
					= genotype_clone.tf_seq[gene_id][copy2][k];
			genotype->tf_seq[gene_id][2][k]
					= genotype_clone.tf_seq[gene_id][copy1][k];
			genotype->tf_seq[gene_id][3][k]
					= genotype_clone.tf_seq[gene_id][copy2][k];
		}
	}
	...
	state->active[gene_id][2] = 0;
	state->active[gene_id][3] = 0;
	...
	/* ACETYLATION_STATE */
	initialize_new_cell_state_change_ids(state, *state_clone, STATE_ZERO,
			state->acetylation_num, state_clone->acetylation_num, copy1, copy2,
			gene_id);
	/* DEACETYLATION_STATE */
	initialize_new_cell_state_change_ids(state, *state_clone, STATE_ZERO,
			state->deacetylation_num, state_clone->deacetylation_num, copy1,
			copy2, gene_id);
	/* PICASSEMBLY_STATE */
	initialize_new_cell_state_change_ids(state, *state_clone, STATE_ONE,
			state->pic_assembly_num, state_clone->pic_assembly_num, copy1,
			copy2, gene_id);
	/* TRANSCRIPTINIT_STATE */
	initialize_new_cell_state_change_ids(state, *state_clone, STATE_TWO,
			state->transcript_init_num, state_clone->transcript_init_num,
			copy1, copy2, gene_id);
	/* PICDISASSEMBLY_STATE */
	initialize_new_cell_state_change_ids(state, *state_clone, STATE_ONE,
			state->pic_disassembly_num, state_clone->pic_disassembly_num,
			copy1, copy2, gene_id);
...
*/
		...
		if (gene_id_queue == gene_id && copy == copy1) {
			...
			add_fixed_event(gene_id, 0, time, &(state->mRNA_transcr_time_end),
					&(state->mRNA_transcr_time_end));
			printf(
					"move mRNATranscrTime event at time=%g on gene %d copy=%d to copy=0 of total mRNA_transcr_num=%d\n",
					time, gene_id, copy1,
					state_clone->mRNA_transcr_num[gene_id][copy1]);
			/* remove from original queue */
			delete_fixed_event(gene_id, copy1, 0,
					&(state_clone->mRNA_transcr_time_end),
					&(state_clone->mRNA_transcr_time_end_last));
			(state_clone->mRNA_transcr_num[gene_id][copy1])--;
			...
			add_fixed_event(gene_id, 1, time, &(state->mRNA_transcr_time_end),
					&(state->mRNA_transcr_time_end));
			printf(
					"move mRNATranscrTime event at time=%g on gene %d copy=%d to copy=1 of total mRNA_transcr_num=%d\n",
					time, gene_id, copy2,
					state_clone->mRNA_transcr_num[gene_id][copy2]);
			/* remove from original queue */
			delete_fixed_event(gene_id, copy2, 0,
					&(state_clone->mRNA_transcr_time_end),
					&(state_clone->mRNA_transcr_time_end_last));
			(state_clone->mRNA_transcr_num[gene_id][copy2])--;
		}
 ...
 */
//DESTROY
void do_cell_division(int mother_id, int daughter_id, int keep_mother,
		int keep_daughter, Genotype *mother, CellState *mother_state,
		GillespieRates *mother_rates, float mother_transport[NGENES],
		float mother_mRNAdecay[NGENES],

		Genotype *daughter, CellState *daughter_state,
		GillespieRates *daughter_rates, float daughter_mRNAdecay[NGENES],
		float daughter_transport[NGENES], float fraction, float x, float dt) {
	int i, j, total;

	/* clone of cell */
	Genotype genotype_clone;
	CellState state_clone;
	//GillespieRates rates_clone; change these references to state_clone
	...
	clone_cell(mother, mother_state, mother_rates, &genotype_clone,
			&state_clone, &rates_clone);
	...
	if (keep_daughter) {
		...
		initialize_new_cell_state(daughter_state, state_clone, daughter_rates,
				fraction);
		...
		printf(
				"daughter cell %03d is founded by cell %03d with %2d divisions\n",
				daughter_id, daughter_state->founder_id,
				daughter_state->divisions);
	...
	if (keep_mother) {
		...
		initialize_new_cell_state(mother_state, state_clone, mother_rates, (1.0
				- fraction));
		mother_state->divisions++; /* update divisions in mother */
		printf(
				"mother   cell %03d is founded by cell %03d with %2d divisions\n",
	...
	for (i = 0; i < NGENES; i++) {
		...
		if (keep_daughter) {
			...
			initialize_new_cell_gene(daughter, genotype_clone, daughter_state,
					&state_clone, daughter_rates, &rates_clone, i,
					daughter_copy1, daughter_copy2, &lastpos_daughter);
		}

		if (keep_mother) {
			LOG_NOCELLID("initialize mother  =%2d, gene_id=%2d\n", mother_id, i);
			initialize_new_cell_gene(mother, genotype_clone, mother_state,
					&state_clone, mother_rates, &rates_clone, i, mother_copy1,
					mother_copy2, &lastpos_mother);
		}
	}
	...
	if (keep_daughter) {
		...
		calc_all_binding_sites(daughter->copies, daughter->cisreg_seq,
				daughter->tf_seq, &(daughter->binding_sites_num),
				&(daughter->all_binding_sites), daughter->hindrance_positions,
				daughter->sites_per_gene, daughter->site_id_pos);
	}

	if (keep_mother) {
		/* recompute *all* binding sites in mother, then relabel sites */
		calc_all_binding_sites(mother->copies, mother->cisreg_seq,
				mother->tf_seq, &(mother->binding_sites_num),
				&(mother->all_binding_sites), mother->hindrance_positions,
				mother->sites_per_gene, mother->site_id_pos);
	}
	...
	if (keep_daughter && keep_mother) {
		...
		if (original_bind_count != mother->binding_sites_num
	...
	for (i = 0; i < NPROTEINS; i++) {
		if (keep_daughter) {
			daughter_state->protein_conc[i] = fraction
					* state_clone.protein_conc[i];
		}
		if (keep_mother) {
			mother_state->protein_conc[i] = (1 - fraction)
					* state_clone.protein_conc[i];
	...
	for (i = 0; i < NGENES; i++) {
		...
		int mRNA_cyto_to_daughter = rint(fraction
		...
		if (keep_mother) {
			mother_state->mRNA_cyto_num[i] = state_clone.mRNA_cyto_num[i]
					- mRNA_cyto_to_daughter;
			LOG_VERBOSE_NOCELLID("mother=%d ", mother_state->mRNA_cyto_num[i]);
		}
		LOG_VERBOSE_NOCELLID("total mRNA_cyto_num=%d\n", state_clone.mRNA_cyto_num[i]);

		/* mRNAs in nucleus */
		int mRNA_nuclear_to_daughter = rint(fraction
				* mother_state->mRNA_nuclear_num[i]);
		if (keep_daughter) {
		...
		if (keep_mother) {
			mother_state->mRNA_nuclear_num[i] = state_clone.mRNA_nuclear_num[i]
					- mRNA_nuclear_to_daughter;
		...
		split_mRNA(&(state_clone.mRNA_transl_time_end),
				&(state_clone.mRNA_transl_time_end_last),
				state_clone.mRNA_transl_cyto_num,
				&(daughter_state->mRNA_transl_time_end),
				&(daughter_state->mRNA_transl_time_end_last),
				daughter_state->mRNA_transl_cyto_num,
				&(mother_state->mRNA_transl_time_end),
				&(mother_state->mRNA_transl_time_end_last),
				mother_state->mRNA_transl_cyto_num, i, fraction);
	}
	fflush(stdout);
	...
	if (keep_mother) {
		...
		recalibrate_cell(mother_rates, mother_state, mother, mother_mRNAdecay,
				mother_transport, dt);
 ...
 */
//DESTROY?
void log_snapshot(CellState *state, Genotype *genotype,
		float mRNAdecay[NGENES], float transport[NGENES], float x, float t) {
	int p, nkon = 0;//i,


	for (p = 0; p < MAX_COPIES; p++) {
		LOG_NOFUNC(" acetylation=%g (copy %d)\n deacetylation=%g (copy %d)\n PIC assembly=%g (copy %d)\n transcriptinit=%g (copy %d)\n",
				(float)state->acetylation_num[p]*ACETYLATE, p, (float)state->deacetylation_num[p]*DEACETYLATE, p,
				(float)state->pic_assembly_num[p]*PICASSEMBLY, p, (float)state->transcript_init_num[p]*TRANSCRIPTINIT, p);
	}
...
float active_to_repress(Genotype indiv, float initProteinConc[NGENES],
		int start, int gene) {
	int n;
	...
	//system("PAUSE");
	qsort((void *) &(indiv.all_binding_sites[start]),
			indiv.sites_per_gene[gene], sizeof(struct AllTFBindingSites),
			(compfn) compare);
	...
	TFBS = 0;
	while (indiv.all_binding_sites[start + TFBS].left_edge_pos < startNum
			+ HIND_LENGTH) {
		TFBS++;
	}
	srand(time(NULL));
	...
	while (val < NITER) {
		...
		while (left_edge_pos[posNext] <= (CISREG_LEN - HIND_LENGTH)) {//loop through all sites in window
			...
			while (indiv.all_binding_sites[structStart + TFBS].left_edge_pos
					< indiv.all_binding_sites[structStart].left_edge_pos
							+ HIND_LENGTH && structStart + TFBS < endNum) {
				TFBS++;
   ...
   //       printf("TFBS: %i\n",TFBS);
			...
			for (lem = 0; lem < TFBS; lem++) {
				...
				arrayWT[lem].startPos = indiv.all_binding_sites[lem
						+ structStart].left_edge_pos;
				arrayWT[lem].hammDist = indiv.all_binding_sites[lem
						+ structStart].hamming_dist;
				arrayWT[lem].tfIDon
						= indiv.all_binding_sites[lem + structStart].tf_id;
				bob = indiv.all_binding_sites[lem + structStart].tf_id;
				arrayWT[lem].conc = initProteinConc[bob];
//ask Jasmin why she commented out next line, which is called by qsort
			...
			qsort((void *) &(arrayWT[0]), TFBS, sizeof(struct Wtype),
					(compfn) compareWeights);
      ...
      int t;
			...
			for (n = 0; n < TFBS + 1; n++) {
				if (n == TFBS) {
					prob[n] = (arrayWT[0].weight) / weightSum;
				} else {
					prob[n] = (arrayWT[n].weight) / weightSum;
				}
			...
			for (n = 0; n < TFBS + 1; n++) {
				if (n == 0) {
					...
					partition[n] = (partition[(n - 1)] + prob[n]);
				} else {
					partition[n] = 1;
				}
			...
			kTF = 0;
			if (0 <= checkP && checkP <= partition[0]) {
				b = 0;
			} else {
				for (k = 0; k < TFBS; k++) {
...
void active_vect(Genotype indiv, float initProteinConc[NGENES],
		float *gene_active) {
	int geneSum = 0;
	int i;
...
//DESTROY
//CHANGE JU
int ready_to_transcribeJU(int gene_id, float *gene_active, CellState *state, int cell_id) {
	//srand(time(NULL));
	float n;
	n = ran1(&seed);
	//n= rand()%10 +1;
 ...
 */
void revise_activity_state(int gene_id, int gene_copy, Genotype *genotype,
		CellState *state, float *gene_active, float transport) {

	int transcriptrule, oldState;
	...
	//system("PAUSE");
	//}
	//start changes here: active[...] * whatever
	/*
	 * first set of rules:
	 * ACTIVATING TFs exceed REPRESSING TFs
	 */
	oldState =  state->active[gene_id][gene_copy];
	int *state_arr = state->stateID_num;
	int numState0 = state->stateID_num[0];
	int numState1 = state->stateID_num[1];
	int numState2 = state->stateID_num[2];
	int activeProb = 1;
	if(gene_active[gene_id]<PROB_ACTIVATING){activeProb = 0;}

	//perform calculations with rates
	float compare0to1 = ACETYLATE * numState0 * gene_active[gene_id];
	float compare1to0 = DEACETYLATE * numState1 * (1-gene_active[gene_id]);
	float compare1to2 = PICASSEMBLY * numState1 * gene_active[gene_id];
	float compare2to1 = transport * numState2;


	 //evaluate items at state zero (first state)
		if(oldState == 0){
			//adjust the number of genes in state zero as well as state one
			if(compare1to0 > compare0to1 && activeProb == 1) {
				//change from state 1 to 0
				state->active[gene_id][gene_copy] = oldState+1;
				state->stateID_num[0] = state->stateID_num[0] - 1;
				state->stateID_num[1] = state->stateID_num[1]+1;
			}
		// evaulate at state one (second state)
		} else if(oldState == 1){
			if(activeProb == 1){
			//adjust the number of genes in state one and state two
				if(compare1to2 > compare2to1) {
					//change from state 1 to 2
					state->active[gene_id][gene_copy] = oldState+1;
					state->stateID_num[1] = state->stateID_num[1] - 1;
					state->stateID_num[2] = state->stateID_num[2]+1;
				}
			}
			//adjust the number of genes in states zero and one
			else if(compare1to0 > compare0to1){
				//change from state 1 to 0
				state->active[gene_id][gene_copy] = oldState - 1;
				state->stateID_num[1] = state->stateID_num[1] - 1;
				state->stateID_num[0] = state->stateID_num[0] + 1;
			}
		//evaulate at state two (third state)
		} else if(oldState == 2){
			//adjust the number of genes in states one and two
				if(compare1to2 > compare2to1) {
					//change from state 1 to 2
					state->active[gene_id][gene_copy] = oldState-1;
					state->stateID_num[2] = state->stateID_num[2] - 1;
		...
		LOG_NOCELLID("New State: %i Gene Active: %f\n",state->active[gene_id][gene_copy],gene_active[gene_id]);
#if 0
	...
	if ((transcriptrule) && oldstate == OFF_FULL) {
		...
		state->state_change_ids[STATE_ZERO][gene_copy][state->acetylation_num[gene_copy]]
				= gene_id;
		//LOG_ERROR("ace num = %d, gene copy = %d\n", rates->acetylation_num[gene_copy], gene_copy);
		(state->acetylation_num[gene_copy])++;
		LOG_ERROR("ace num = %d\n", state->acetylation_num[gene_copy]);
	}
	...
	if (!(transcriptrule) && oldstate == ON_NO_PIC) {
		...
		remove_from_array(gene_id, STATE_ONE,
				state->state_change_ids[STATE_ONE][gene_copy],
				&(state->pic_assembly_num[gene_copy]), (int) 0);
		state->state_change_ids[STATE_ZERO][gene_copy][state->deacetylation_num[gene_copy]]
				= gene_id;
		(state->deacetylation_num[gene_copy])++;
	...
	if ((state->active[gene_id][gene_copy] == OFF_PIC
			|| state->active[gene_id][gene_copy] == ON_FULL)) {//&& numactive==0
 ...
 */
//DESTROY
int do_single_timestep(Genotype *genotype, CellState *state,
		float *t, float transport[NGENES],
		float mRNAdecay[NGENES], float *x, float *dt,
		TimeCourse *timecoursestart[NPROTEINS],
		TimeCourse *timecourselast[NPROTEINS], int maxbound2, int maxbound3,
		int no_fixed_dev_time, float *genesActive) {
	...
	if (*t > 0.00005 && genotype->burn_in) {
		printf("recalibrating cell %3d after burn-in!\n", genotype->cell_id);

		recalibrate_cell(state, genotype, mRNAdecay, transport, *dt);
		...
		for (j = 0; j < NGENES; j++) {
			...
			revise_activity_state(j, genotype->all_binding_sites->gene_copy,
					genotype, state, genesActive,transport[j]);
		}

		genotype->burn_in = 0; /* now disable burn-in */
	}

	/* compute S-phase offsets */
	if (critical_size > 0.0 && state->cell_size >= critical_size
			&& !state->in_s_phase) { /* run until checkpoint reached */
	...
	//LOG_ERROR("random x before = %f\n", *x);
	//LOG_ERROR("seed: %lu\n",seed);
	*x = expdev(&seed); /* draw random number */
	//LOG_ERROR("random x after = %f\n", *x);
	...
	*dt = .03 * (*x);
	...
	event = does_fixed_event_end(state->mRNA_transl_time_end,
			state->mRNA_transcr_time_end, state->replication_time_end,
			fixed_time);
	...
	while (event > 0) {
		...
		case 1: /* if a transcription event ends */
			//printf("case 1");
			end_transcription(dt, *t, state, transport);

			update_protein_conc_cell_size(state->protein_conc, state, genotype,
					*dt, *t, timecoursestart, timecourselast,
					state->protein_conc);
			/*active_vect(genotype[0], state->protein_conc, genesActive);
			 for(j=0; j<NGENES; j++){
			 revise_activity_state(j,genotype->all_binding_sites->gene_copy, genotype, state,rates, genesActive);
		...
		case 2: /* if a translation event ends */
			...
			*dt = state->mRNA_transl_time_end->time - *t; /* make dt window smaller */
			...
			for (i = 0; i < NGENES; i++)
				total += state->mRNA_transl_cyto_num[i];
			...
			delete_fixed_event_start(&(state->mRNA_transl_time_end),
					&(state->mRNA_transl_time_end_last));
		...
		case 3: /* replicate gene */
			...
			*dt = state->replication_time_end->time - *t; /* make dt window smaller */
			...
			replicate_gene(state, genotype,
					state->replication_time_end->gene_id, *t);// kon_states, koffvalues,

			/* delete the event that just happened */
			delete_fixed_event_start(&(state->replication_time_end),
					&(state->replication_time_end_last));
		...
		*dt = expdev(&seed);
		...
		fixed_time = no_fixed_dev_time ? (*t + *dt) : fminf(*t + *dt,
				tdevelopment);
	...
	if (*t + *dt < tdevelopment || no_fixed_dev_time) {
		...
		if (!(state->total > 0.0)) {//+ *konrate
			recalibrate_cell(state, genotype, mRNAdecay, transport, *dt); // kon_states,&koffvalues
		}
		...
		LOG_ERROR("transport = %f, mrnadecay = %f, pic dis = %f,\n",state->transport, state->mRNAdecay, state->pic_disassembly);
		for (i = 0; i < MAX_COPIES; i++) {
			LOG_ERROR("ace_num[%d] = %d, deace_num[%d] = %d, pic[%d] = %d, trans_num[%d] = %d, pic_dis[%d] = %d\n",
					i,state->acetylation_num[i],i,state->deacetylation_num[i],i, state->pic_assembly_num[i],i, state->transcript_init_num[i],i, state->pic_disassembly_num[i]);
		}
		*/

		*x = ran1(&seed) * state->total;//*(876.76);//*(rates->subtotal);  // + *konrate
		float test2, *testProb, *deaceProb;
		testProb = malloc(NGENES * sizeof(float));
		deaceProb = malloc(NGENES * sizeof(float));
		for (i = 0; i < NGENES; i++) {
			testProb[i] = genesActive[i] * ACETYLATE * (float) sum_rate_counts(
					state->acetylation_num) * 10;
		}

		for (i = 0; i < NGENES; i++) {
			if (test2 < testProb[i]) {
				revise_activity_state(i,
						genotype->all_binding_sites->gene_copy, genotype,
						state, genesActive,transport[i]);
			}
		}
		...
		if (verbose) {
			///log_snapshot(state, genotype, mRNAdecay, transport, *x, *t);
		}

		float test;
		test = active_to_repress(*genotype, state->protein_conc, 0, 0);

		if (*x < state->transport) {

			transport_event(state, genotype, transport, timecoursestart,
					timecourselast, *dt, *t, *x);//kon_states,
			...
			*x -= state->transport;
			 ...
			 */
			if (*x < state->mRNAdecay) {

				///LOG_ERROR("decay event\n");
				mRNA_decay_event(state, genotype, mRNAdecay, timecoursestart,
						timecourselast, *dt, *t, *x);//kon_states,
				...
				*x -= state->mRNAdecay;
				 ...
				 */
				if (*x < state->pic_disassembly) {

					///LOG_ERROR("pic disassembly event\n");
					disassemble_PIC_event(state, genotype, timecoursestart,
							timecourselast, *dt, *t, *x);//kon_states,
				} else {

					*x -= state->pic_disassembly;

					//LOG_ERROR("rates salphc = %f\n", rates->salphc);
					///LOG_ERROR("transcript init num = %d, pic assemb = %d, pic assem *PIC = %f\n", sum_rate_counts(state->transcript_init_num), sum_rate_counts(state->pic_assembly_num), sum_rate_counts(state->pic_assembly_num)*PICASSEMBLY);
					///LOG_ERROR("CHECK! x = %f, sum_rate_counts = %d, acetylate = %f\n", *x, sum_rate_counts(state->acetylation_num), ACETYLATE );

					if (*x < (float) sum_rate_counts(state->acetylation_num)
							* ACETYLATE) {

						histone_acteylation_event(state, genotype,
								timecoursestart, timecourselast, *dt, *t);
						//active_vect(genotype[0], state->protein_conc, genesActive);
					} else {
						*x -= (float) sum_rate_counts(state->acetylation_num)
								* ACETYLATE;

						if (*x < (float) sum_rate_counts(
								state->deacetylation_num) * DEACETYLATE) {
							///LOG_ERROR("deact event\n");
							histone_deacteylation_event(state, genotype,
									timecoursestart, timecourselast, *dt, *t);

							//active_vect(genotype[0], state->protein_conc, genesActive);
						} else {

							*x -= (float) sum_rate_counts(
									state->deacetylation_num) * DEACETYLATE;
							/*
							...
							if (*x < (float) sum_rate_counts(
									state->pic_assembly_num) * PICASSEMBLY) {
								assemble_PIC_event(state, genotype,
										timecoursestart, timecourselast, *dt,
										*t);
								//active_vect(genotype[0], state->protein_conc, genesActive);
							} else {
								...
								*x -= (float) sum_rate_counts(
								 ...
								 */
								///LOG_ERROR("transcript init num = %d\n", sum_rate_counts(state->transcript_init_num));

								if (*x < (float) sum_rate_counts(
										state->transcript_init_num)
										* TRANSCRIPTINIT) {
									///LOG_ERROR("transcript init event time = %f\n", *t);
									transcription_init_event(state, genotype,
									...
									///		genotype->cell_id, *t, *x, state->total);

									///log_snapshot(state, genotype,
									///		mRNAdecay, transport, *x, *t);//kon_states, &koffvalues, *konrate,
									recalibrate_cell(state, genotype,
											mRNAdecay, transport, *dt); //kon_states, &koffvalues,
									///log_snapshot(state, genotype, mRNAdecay,
									///		transport, *x, *t);//kon_states, &koffvalues, *konrate,
 ...
 */
//DESTROY
void init_run_pop(Genotype genotype[POP_SIZE], CellState state[POP_SIZE],
		TimeCourse *timecoursestart[POP_SIZE][NGENES],
		TimeCourse *timecourselast[POP_SIZE][NGENES], float temperature, /* in Kelvin */
		float kdis[NUM_K_DISASSEMBLY], int output_binding_sites,
		int no_fixed_dev_time, int max_divisions) {
	...
	for (j = 0; j < POP_SIZE; j++) {
		...
		initialize_cell(&state[j], &genotype[j], genotype[j].cell_id, genotype[j].copies, genotype[j].mRNAdecay, init_mRNA, init_protein_conc, burn_in);
		//printf("proteindecay[0] = %f\n", genotype[j].proteindecay[]);

		/* print binding sites */
		if (output_binding_sites)
			print_all_binding_sites(genotype[j].copies,
					genotype[j].all_binding_sites,
					genotype[j].binding_sites_num, genotype[j].tf_seq,
					genotype[j].cisreg_seq, genotype[j].site_id_pos);
		...
		add_time_points((float) 0.0, state[j].protein_conc, timecoursestart[j],
				timecourselast[j]);
		...
		//system("PAUSE");
		calc_from_state(&genotype[j], &state[j], transport[j],
				mRNAdecay[j]);
		...
		if ((start = clock()) != -1) {
			do_single_timestep(&(genotype[j]), &(state[j]), &(t[j]),
					transport[j], mRNAdecay[j], &(x[j]), &(dt[j]),
					timecoursestart[j], timecourselast[j], maxbound2,
					maxbound3, no_fixed_dev_time, genesActive);
			stop = clock();
			tt = (double) (stop - start) / CLOCKS_PER_SEC;
			//printf("Run time: %f\n", tt);
			//system("PAUSE");
	...
	(!no_fixed_dev_time && t_next < tdevelopment)) { /* or, if fixed dev time, run until tdevelopment reached */
		...
		cell_status = do_single_timestep(&(genotype[cell]), &(state[cell]),
				&(t[cell]), transport[cell], mRNAdecay[cell],
				&(x[cell]), &(dt[cell]), timecoursestart[cell],
				timecourselast[cell], maxbound2, maxbound3, no_fixed_dev_time,
				genesActive);
		...
		if (t[cell] >= (state[cell]).division_time) { /* we have now reached cell division */
			...
			if (state[cell].cell_size > 2.0) {
				large_cell_size++;
				printf(
						"[cell %03d] at t=%g cell size =%g, exceeding Y=2.0, %d of %d divisions (%g fraction)\n",
						cell, t[cell], state[cell].cell_size, large_cell_size,
						divisions, (double) large_cell_size
								/ (double) divisions);
			...
			printf(
					"[cell %03d] (size=%g) dividing into mother=%03d and daughter=%03d at t=%g, division=%g, total divisions=%d (t_next=%g)\n",
					cell, state[cell].cell_size, mother_id, daughter_id,
					t[cell], current_division_time, divisions, t_next);
			...
			//TODO: Fix this
			/*
			do_cell_division(mother_id, daughter_id, keep_mother,
					keep_daughter, &(genotype[mother_id]), &(state[mother_id]),
					transport[mother_id],
					mRNAdecay[mother_id], &(genotype[daughter_id]),
					&(state[daughter_id]),
					transport[daughter_id], mRNAdecay[daughter_id], 0.44,
					x[mother_id], dt[mother_id]);
			*/
			...
			if (keep_mother) {
				...
				cell_status = do_single_timestep(&(genotype[mother_id]),
						&(state[mother_id]),
						&(t[mother_id]), transport[mother_id],
						mRNAdecay[mother_id], &(x[mother_id]),
						&(dt[mother_id]), timecoursestart[mother_id],
						timecourselast[mother_id], maxbound2, maxbound3,
						no_fixed_dev_time, genesActive);

				if (cell_status == -1) { /* if cell dies, add it to the reaper queue */
					insert_with_priority_heap(reaper_queue, mother_id,
							TIME_INFINITY);
					...
					///		mother_id, t[mother_id], reaper_queue->n, priority_queue->n);
					insert_with_priority_heap(priority_queue, mother_id,
							t[mother_id]);
				}

			} else {
				printf(
						"don't update mother [%03d] it replaced by daughter [%03d]\n",
						mother_id, daughter_id);
			...
			if (keep_daughter) { /* only update if daughter cell isn't replaced by mother */
				...
				cell_status = do_single_timestep(&(genotype[daughter_id]),
						&(state[daughter_id]),
						&(t[daughter_id]), transport[daughter_id],
						mRNAdecay[daughter_id], &(x[daughter_id]),
						&(dt[daughter_id]), timecoursestart[daughter_id],
						timecourselast[daughter_id], maxbound2, maxbound3,
						no_fixed_dev_time, genesActive);

				if (cell_status == -1) { /* if cell dies, add to reaper queue */
					insert_with_priority_heap(reaper_queue, daughter_id,
							TIME_INFINITY);
					...
					///		daughter_id, t[daughter_id], reaper_queue->n, priority_queue->n);
					insert_with_priority_heap(priority_queue, daughter_id,
							t[daughter_id]);
				}
			} else {
				printf(
						"don't update daughter [%03d] it is replaced by mother [%03d]\n",
						daughter_id, mother_id);
	...
	for (j = 0; j < POP_SIZE; j++) {
		...
		////		genotype[j].founder_id, genotype[j].divisions,
		////		t[j] == TIME_INFINITY ? " [dead]" : "");
		///LOG_NOCELLID("cell %03d derived from founder %03d had %2d divisions%s\n",
		///		j, genotype[j].founder_id, genotype[j].divisions, t[j] == TIME_INFINITY ? " [dead]": "");
	}
...
void print_time_course(TimeCourse *start, int i, int j) {
	FILE *fpout;
	char filename[80];
...
void print_all_protein_time_courses(
		TimeCourse *timecoursestart[POP_SIZE][NPROTEINS],
		TimeCourse *timecourselast[POP_SIZE][NPROTEINS]) {
	int i, j;
	for (j = 0; j < POP_SIZE; j++) {
		for (i = 0; i < NPROTEINS; i++) {
			if ((output))
				print_time_course(timecoursestart[j][i], i, j);
		}
	}
}
