    ...
    for (i=0; i < NGENES; i++) {
      if (genotype->copies[i] > j) {  
        ...
        LOG_VERBOSE("Initializing statechange gene=%d, ploidy=%d state_change_ids[%d][%d]=%d\n", i, j, j, 
                    pos, state->state_change_ids[STATE_ZERO][j][pos]);
        pos++;
      }
    }
  }
                     ...
                     GillespieRates *rates)
{
  ...
  remove_from_array(gene_id, STATE_TWO, state->state_change_ids[STATE_TWO][gene_copy], 
                    &(rates->transcript_init_num[gene_copy]), (int) 1);
  remove_from_array(gene_id, STATE_ONE, state->state_change_ids[STATE_ONE][gene_copy], 
  ...
  if (state->active[gene_id][gene_copy] == OFF_PIC) {
    ...
    state->state_change_ids[STATE_ZERO][gene_copy][rates->deacetylation_num[gene_copy]] = gene_id;
                           ...
                           float *gene_active)
{
 ...
 /* transcriptrule = ready_to_transcribe(gene_id, gene_copy, 
  ...
  if ((transcriptrule) && oldstate==OFF_FULL){
    ...
    state->state_change_ids[STATE_ZERO][gene_copy][rates->acetylation_num[gene_copy]] = gene_id;
 ...
 // LOG_ERROR("transcript rule = %d, oldstate = %d, OFF_NO_PIC = %d\n", transcriptrule, oldstate, OFF_NO_PIC);
  ...
  if ((transcriptrule) && oldstate==OFF_NO_PIC) {
                       ...
                       LOG_ERROR("OFF to ON\n");
    state->active[gene_id][gene_copy] = ON_NO_PIC;
    remove_from_array(gene_id, STATE_ZERO,  state->state_change_ids[STATE_ZERO][gene_copy], 
                               ...
                               float dt, float t)
{
  ...
  state->active[gene_id][gene_copy] = ON_NO_PIC;
  remove_from_array(gene_id, STATE_ZERO, state->state_change_ids[STATE_ZERO][gene_copy], &(rates->acetylation_num[gene_copy]), (int) 1);
 // state->state_change_ids[PICASSEMBLY_STATE][gene_copy][rates->pic_assembly_num[gene_copy]] = gene_id; 
    //(rates->pic_assembly_num[gene_copy])++;
  //if (is_one_activator(gene_id, gene_copy, //state->tf_bound_indexes, state->tf_bound_num, 
    ...
    state->state_change_ids[STATE_ONE][gene_copy][rates->pic_assembly_num[gene_copy]] = gene_id; 
    (rates->pic_assembly_num[gene_copy])++;
  //}
}
                                 ...
                                 float dt, float t)
{
  ...
  state->active[gene_id][gene_copy] = OFF_FULL;
  remove_from_array(gene_id, STATE_ZERO, state->state_change_ids[STATE_ZERO][gene_copy], &(rates->deacetylation_num[gene_copy]), (int) 1);
}
                      ...
                      float dt, float t)
{
 ...
 LOG_ERROR("PIC assembly event gene %d copy %d\nstate change from %d to 6\n",
  ...
  state->state_change_ids[STATE_TWO][gene_copy][rates->transcript_init_num[gene_copy]] = gene_id;
  (rates->transcript_init_num[gene_copy])++;
  state->state_change_ids[STATE_ONE][gene_copy][rates->pic_disassembly_num[gene_copy]] = gene_id;
                           ...
                           float dt, float t, float x)
{
  ...
  while (j < NGENES*current_ploidy && x>0) {
    ...
    x -= genotype->pic_disassembly[state->state_change_ids[STATE_ONE][gene_copy][gene_loc]][gene_copy];
  }
                    ...
                    float t) 
{
 ...
 // shift_binding_site_ids(state, kon_states, end_tfbs_pos + 1, offset);
  ...
  for (i=0; i < current_ploidy; i++) {
    p = i + current_ploidy;
    state->state_change_ids[STATE_ZERO][p][rates->acetylation_num[p]] = gene_id;
    ...
    LOG_VERBOSE("[gene %2d] [clone acetylation]: ploidy=%d state_change_ids[%d][%d]=%d\n", gene_id, p, p, 
                rates->acetylation_num[p], state->state_change_ids[STATE_ZERO][p][rates->acetylation_num[p]]);
                ...
                GillespieRates *rates_clone)
{
  ...
  for (i=0; i < NGENES; i++) {
    for (p=0; p < MAX_COPIES; p++) {
      ...
      state_clone->state_change_ids[STATE_ZERO][p][i] = state_orig->state_change_ids[STATE_ZERO][p][i];
      state_clone->state_change_ids[STATE_ZERO][p][i] = state_orig->state_change_ids[STATE_ZERO][p][i];
      state_clone->state_change_ids[STATE_ONE][p][i] = state_orig->state_change_ids[STATE_ONE][p][i];
      state_clone->state_change_ids[STATE_TWO][p][i] = state_orig->state_change_ids[STATE_TWO][p][i];
      state_clone->state_change_ids[STATE_TWO][p][i] = state_orig->state_change_ids[STATE_TWO][p][i];
