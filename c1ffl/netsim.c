                ...
                float signal_on_strength)
{
    ...
    {
        flag='o'; 
        state->protein_number[N_SIGNAL_TF-1]=signal_on_strength;    //always start with signal off
        #if N_SIGNAL_TF==2
            state->protein_number[0]=background_signal_strength;
        #endif  
//        add_fixed_event(-1,t+delay,&(state->signal_on_head),&(state->signal_on_tail));
//        flag='o';
//        t=t+delay;
...
//        remove_edges_iteratively(&genotype_clone);    
        ...
        {	 
            ...
            signal_profile=NULL;             
#endif            
            /*set exponentially distributed delay that has mean=10min and max=60min */
            delay=0.0;
//            while(delay>30.0)
//                delay=10.0*expdev(RS_parallel[thread_ID]);
...
#else
            for(i=0;i<genotype->ngenes;i++)
                phenotype[i][*timepoint]=state->gene_specific_protein_number[i]; 
#endif
...
{     
...
#else
    float phenotypeA[N_REPLICATES][genotype->ngenes][N_TIMP_POINTS];
    float phenotypeB[N_REPLICATES][genotype->ngenes][N_TIMP_POINTS];
#endif
    ...
    #pragma omp parallel num_threads(N_THREADS)
    {
        ...
        {             
            ...
            delay=0.0;
//            while(delay>30.0)
//                delay=10.0*expdev(RS[thread_ID]);
            initialize_cell(&genotype_clone,
                            &state_clone,
                            mRNA, 
                            protein,                     
                            env1_t_development+delay);
            set_signal(&state_clone,delay,env1_t_signal_on,env1_t_signal_off,signal_profile,env1_t_development,env1_signal_strength);        
            state_clone.t = 0.0;
            calc_all_rates(&genotype_clone, &state_clone, &rate_clone, env1_t_development+delay,INITIALIZATION, thread_ID);//,N_updateA[thread_ID*N_replicates_per_thread+i],max_changeA[thread_ID*N_replicates_per_thread+i]);
            timepoint=0;
            while(state_clone.t<env1_t_development+delay && end_state==1)
            {    
...
//                delay=10.0*expdev(RS[thread_ID]);
            initialize_cell(&genotype_clone,
                            ...
                            mRNA, 
                            protein,                     
                            env2_t_development+delay);
            set_signal(&state_clone,delay,env2_t_signal_on,env2_t_signal_off,signal_profile,env2_t_development,env2_signal_strength);        
            state_clone.t = 0.0;
            calc_all_rates(&genotype_clone, &state_clone, &rate_clone, env2_t_development+delay,INITIALIZATION,thread_ID);//N_updateB[thread_ID*N_replicates_per_thread+i],max_changeB[thread_ID*N_replicates_per_thread+i]);        
            timepoint=0;
            while(state_clone.t<env2_t_development+delay && end_state==1)
            {
                do_single_timestep_plotting(&genotype_clone, 
                                            &state_clone, 
                                            &rate_clone,                                                                                                                                       
    ...
    {
        snprintf(filename1,sizeof(char)*32,"proteinA_%i",l);
        snprintf(filename2,sizeof(char)*32,"proteinB_%i",l);
    ...
    {
        snprintf(filename1,sizeof(char)*32,"geneA_%i",l);
        snprintf(filename2,sizeof(char)*32,"geneB_%i",l);
        fp1=fopen(filename1,"w");
        fp2=fopen(filename2,"w");
        for(m=0;m<N_TIMP_POINTS;m++)
        {
                        ...
                        RngStream RS_main)
{
    ...
    {       
        ...
        clone_genotype(genotype_ori_copy,genotype_ori);         
        calc_all_binding_sites(genotype_ori);
        find_motifs(genotype_ori); 
        print_motifs(genotype_ori);        
...
void run_plotting(  Genotype *genotype_ori,
                    Genotype *genotype_ori_copy,
                    int init_mRNA[MAX_GENES],
                    float init_protein_number[MAX_GENES],
                    RngStream RS_parallel[N_THREADS],
                    Mutation *mut_record,
                    FILE *fp,
                    int replay_N_steps)
                                ...
                                int replay_N_steps)
{
    ...
    FILE *f_aft_perturbation,*f_bf_perturbation;   
    ...
    {               
        ...
        fscanf(fitness_record,"%d %d %d %d %c %f %f %f %f %f %f %f %d %d %d %d\n",
                ...
                &int_buffer,
                &int_buffer);
        if(i>=replay_N_steps-9999)
        {            
            calc_all_binding_sites(genotype_ori);
            find_motifs(genotype_ori); 
#if DIRECT_REG
            if(genotype_ori->N_motifs[7]!=0 && genotype_ori->N_motifs[7]==genotype_ori->N_motifs[0])
#else          
...
//        fclose(fp); 
        ...
        if(init_step<BURN_IN)
        {
            ...
            output_genotype(genotype_ori, BURN_IN);
        #if OUTPUT_RNG_SEEDS
    ...
    {             
        ...
        {	
            ...
            fprintf(fp,"%.10f %.10f\n",
                    ...
                    sqrt(genotype_ori_copy->sq_SE_fitness));
            fclose(fp);     
#endif
...
{  
    ...
    for(i=N_SIGNAL_TF; i < MAX_GENES; i++) /* loop through tf genes*/
    {
        init_mRNA[i]=0;
    ...
    {           
        ...
        {                     
            ...
            calc_fitness_stats(&genotype_ori,&(GR1[0]),&(GR2[0]),recalc_new_fitness);
            fp=fopen(output_file,"a+");
            fprintf(fp,"step N_tot_mut_tried N_mut_tried_this_step N_hit_bound fixed_mutation selection_coeff fitness avg_GR1 avg_GR2 se_fitness se_GR1 se_GR2 N_genes N_proteins N_activator N_repressor\n");
...
#endif
#if COUNT_LONG_ARM
    int copies_reg_by_env2[MAX_GENES], N_copies_reg_by_env2, activators_copy[MX_PROTEINS], protein_id2;
#endif
...
#if DIRECT_REG
        temp=1.0;
    #if FORCE_FAST_TF_CONTROLLED /* add a binding site of the signal to gene_id.*/        
        for(i=0;i<genotype->binding_sites_num[gene_id];i++)
        {
